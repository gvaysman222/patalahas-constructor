...
(function initConstructorCheckoutTemplateConfig(global) {
  ...
  const centerEmbroideryFonts = [
    ...
    },
    {
      ...
      fontWeight: 400,
      fontSizeAdjust: 0.7,
      lowercasePreviewScale: 1.3,
      lowercaseExportScale: 1.3,
      lowercaseLineHeightScale: 0.3,
    },
  ];
  const centerPrintFonts = [
    ...
    },
    {
      ...
      lineHeightScale: 0.65,
      fontWeight: 400,
      fontSizeAdjust: 0.7,
      lowercasePreviewScale: 2.0,
      lowercaseExportScale: 2.0,
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const KONVA_TEMPLATE_CONFIGS = {
      ...
      'polaroid-diagonal-text': {
        ...
        textScaleMultiplier: 1.381,
        printSizeOverrides: {
          s: { photoExportX: 457.1, photoExportY: 486.9, photoScaleX: 0.782, photoScaleY: 0.802, frameExportX: 453, frameExportY: 483, frameScale: 0.947, diagonalExportX: 538.7, diagonalExportY: 605 },
      ...
      'polaroid-inside-text': {
        ...
        printSizeOverrides: {
          s: { photoExportX: 457.1, photoExportY: 486.9, photoScaleX: 0.782, photoScaleY: 0.802, frameExportX: 453, frameExportY: 483, frameScale: 0.947 },
          's:l': { photoExportX: 448, photoExportY: 488, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 483, frameScale: 0.822, exportX: 553.1, exportCenterY: 623.6, textScaleMultiplier: 0.45 },
        },
        layerOrder: ['polaroidFrame', 'photo', 'text'],
      },
    };
    ...
    function textHasLowercase() {
      ...
      const result = text.length > 0 && text !== text.toUpperCase();
      console.log('[DEBUG textHasLowercase]', JSON.stringify({ text: text.substring(0, 30), upper: text.toUpperCase().substring(0, 30), result }));
      return result;
    }
    function getPreviewFontScale(fontId = state.font, template = getSelectedTemplate()) {
      ...
      if (!opt) return 1;
      const hasLower = textHasLowercase();
      const scale = (hasLower && opt.lowercasePreviewScale != null) ? opt.lowercasePreviewScale : (opt.previewScale || 1);
      console.log('[DEBUG getPreviewFontScale]', JSON.stringify({ fontId, hasLower, lowercasePreviewScale: opt.lowercasePreviewScale, previewScale: opt.previewScale, result: scale }));
      return scale;
    }
    function getExportFontScale(fontId = state.font, template = getSelectedTemplate()) {
      ...
      if (!opt) return 1;
      const hasLower = textHasLowercase();
      const scale = (hasLower && opt.lowercaseExportScale != null) ? opt.lowercaseExportScale : (opt.exportScale || 1);
      console.log('[DEBUG getExportFontScale]', JSON.stringify({ fontId, hasLower, lowercaseExportScale: opt.lowercaseExportScale, exportScale: opt.exportScale, result: scale }));
      return scale;
    }
    function getLineHeightScale(fontId = state.font, template = getSelectedTemplate()) {
      ...
      if (!opt) return 1;
      const hasLower = textHasLowercase();
      return (hasLower && opt.lowercaseLineHeightScale != null) ? opt.lowercaseLineHeightScale : (opt.lineHeightScale || 1);
    }
