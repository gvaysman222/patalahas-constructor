...
(function initConstructorCheckoutTemplateConfig(global) {
  ...
  const PHOTO_TOP_TEXT_PREVIEW_GAP = 8;
  const PHOTO_TOP_TEXT_EXPORT_GAP = 10;
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function getTemplatePhotoLayout(
      ...
      if (isPhotoTopTextTemplate(template)) {
        ...
        const previewTextHeight = (
          previewFontSize
          + (Math.max(0, resolvedLineCount - 1) * previewFontSize * previewLineHeight)
        ) * previewScale;
        const previewPhotoTopPercent = Number.isFinite(previewTopPercent)
          ...
          : ((Number(textMetrics.y) + (Math.max(0, resolvedLineCount - 1) * Number(textMetrics.lineHeight)) + PHOTO_TOP_TEXT_EXPORT_GAP) / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100;
        const exportY = Math.round(
          Number(textMetrics.y)
          + (Math.max(0, resolvedLineCount - 1) * Number(textMetrics.lineHeight))
          + PHOTO_TOP_TEXT_EXPORT_GAP
        );
