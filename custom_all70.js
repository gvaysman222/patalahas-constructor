...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    // --- Konva Engine Integration ---
    const KonvaEngine = window.KonvaConstructorEngine;
    const konvaPreview = KonvaEngine ? new KonvaEngine.PreviewRenderer() : null;
    const konvaExport = KonvaEngine ? new KonvaEngine.ExportRenderer() : null;

    /**
     * Build scene params from current constructor state for KonvaEngine.resolveScene().
     * This is the single bridge between the old closure state and the new engine.
     */
    function buildKonvaSceneParams(options = {}) {
      ...
      const hasUpload = Boolean(activeUpload);
      const isEmb = ['emb-center', 'emb-left'].includes(selectedTemplate?.kind);
      const hasStdUpload = hasUpload && !isEmb;

      const photoExportTop = selectedPhotoPrintSize?.exportTop
        ?? photoConfig?.exportTop
        ?? 430;
      ...
      return {
        ...
        exportFontSize: textRules.exportFontSize || DEFAULT_PREVIEW_TEXT_RULES.exportFontSize,
        exportLineHeight: textRules.exportLineHeight || DEFAULT_PREVIEW_TEXT_RULES.exportLineHeight,
        exportFontScale: getExportFontScale(state.font, selectedTemplate),
        fontWeight: textRules.fontWeight || DEFAULT_PREVIEW_TEXT_RULES.fontWeight,
        letterSpacing: getPreviewTextLetterSpacingValue(selectedTemplate, selectedPrintSize, (textRules.exportFontSize || 48) * getExportFontScale(state.font, selectedTemplate)),
        ...
        photoCrop: {
          x: state.cropPosition.x,
          y: state.cropPosition.y,
          zoom: state.cropZoom,
          grayscale: state.photoGrayscale,
        },
        frameSrc: isPolaroidPhotoTemplate(selectedTemplate)
          ? normalizeExportImageUrl(POLAROID_FRAME_IMAGE_URL)
          : null,
        baseImageSrc: normalizeExportImageUrl(getCurrentProductImage()),
        baseImageScale: getProductBaseImageScale(state.product),
        hasUpload: hasStdUpload || (hasUpload && !isEmb),
        isCustom: Boolean(state.selectedCustom),
        hideDesign: state.step === 1,
        textExportOverrides: {
          exportX: textRules.exportX,
          exportCenterY: textRules.exportCenterY,
          exportCenterYWithUpload: textRules.exportCenterYWithUpload,
          anchor: textRules.exportAnchor,
        },
    ...
    function buildTemplatePreviewSvgMarkup(template, mode = 'template') {
      if (!template) return '';
      // Delegate to Konva engine for template preview SVGs
      if (KonvaEngine) {
        return KonvaEngine.buildTemplatePreviewSvg(template.kind, mode);
      }
      // Fallback: basic SVG if engine not loaded
      return `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><rect width="100" height="100" rx="18" fill="#f4f5f7"/></svg>`;
    }
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function createCheckoutCompositePreviewUiSourceUrl() {
      ...
      if (konvaExport && KonvaEngine) {
        try {
          ...
          const scene = KonvaEngine.resolveScene(exportParams);
          const dataUrl = await konvaExport.toDataURL(scene, {
            mimeType: 'image/png',
            quality: 0.92,
          });
    ...
    async function createCheckoutCompositePreviewFile(type = 'image/jpeg', quality = 0.92) {
      ...
      if (konvaExport && KonvaEngine) {
        ...
        try {
          const sceneParams = buildKonvaSceneParams();
          // For export, we need the base image as a data URL to avoid CORS
          const baseImageDataUrl = sceneParams.baseImageSrc
            ? await fetchImageAsDataUrl(sceneParams.baseImageSrc)
            : null;
          const frameDataUrl = sceneParams.frameSrc
          ...
          if (dataUrl) {
            const blob = KonvaEngine.dataURLToBlob(dataUrl);
            const file = blob
              ? new File([blob], buildCheckoutCompositeFileName(extension), { type })
              : null;
...
function renderLeftSummary() {
    ...
    function renderVisualizer() {
      ...
      const designLayer = document.getElementById('designLayer');
      if (konvaPreview && KonvaEngine && designLayer) {
        const sceneParams = buildKonvaSceneParams();
        // Build scene without base image for the design overlay (base image is the DOM <img>)
        const overlayParams = { ...sceneParams, baseImageSrc: null };
        const scene = KonvaEngine.resolveScene(overlayParams);
    ...
    window.addEventListener('resize', () => {
      if (konvaPreview) {
        const designLayer = document.getElementById('designLayer');
        if (designLayer) konvaPreview.resize(designLayer);
      }
    });
