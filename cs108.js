...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const KONVA_TEMPLATE_CONFIGS = {
      ...
      'simple-photo': {
        ...
        photoDefaults: { exportX: 455, exportY: 490, scale: 0.8 },
        printSizeOverrides: {
          l: { photoExportX: 437, photoExportY: 490, photoScale: 0.8 },
        },
        layerOrder: ['photo'],
      },
      'photo-top-text': {
    ...
    function konvaResolveScene(params) {
      const {
        ...
        textExportOverrides = {}, printSizeId = '', printScale = 1,
      } = params;
      ...
      const textScaleMul = psOverride.textScaleMultiplier || config.textScaleMultiplier || 1;
      ...
      if (config.hasPhoto && !config.hasFrame) {
        ...
        const photoOverride = psOverride;
        const photoScale = photoOverride.photoScale ?? pd.scale ?? 1;
        const scaledW = photoExportWidth * photoScale;
        const scaledH = photoExportHeight * photoScale;
      ...
      if (config.hasText && textLines.length > 0) {
        ...
        if (config.textPosition === 'inside-frame' && config.insideText && frameLayout) {
          ...
          textLayout = { x: ex + ew - padX, y: innerY + topInset + ff * it.baselineRatio, fontSize: ff, lineHeight: fl, anchor: 'end', rotation: 0, clipRect: KONVA_DEV_MODE ? null : { x: ex, y: ey, width: ew, height: eh } };
    ...
    function konvaUpdateDebugPanel(node) {
      ...
      const currentPsId = isPhotoNode ? (state.photoPrintSize || '') : (state.printSize || '');
      const psOvr = (tplConfig?.printSizeOverrides && tplConfig.printSizeOverrides[currentPsId]) || {};
      const pd = tplConfig?.photoDefaults || {};
      const configMul = isPhotoNode
        ? (psOvr.photoScale ?? pd.scale ?? 1)
        : (psOvr.textScaleMultiplier || tplConfig?.textScaleMultiplier || 1);
      // Combined scale = config multiplier × node transform scale
      const combinedSx = Math.round(configMul * nodeSx * 1000) / 1000;
      const combinedSy = Math.round(configMul * nodeSy * 1000) / 1000;
      ...
      panel.textContent = `[${name}]\nx: ${x}  y: ${y}\nw: ${w}  h: ${h}\nscale: ${combinedSx} × ${combinedSy}` + (configMul !== 1 ? `\n  (cfg: ${configMul} × node: ${nodeSx})` : '') + `\nrot: ${r}°`;
    ...
    function konvaBuildPlaceholder(layer, el, draggable) {
      ...
      const lines = el.polaroid ? [] : ['ТУТ БУДЕТ', 'ВАШЕ ФОТО'];
      // Background rect — listening:true so Group is hittable for drag
      group.add(new Konva.Rect({
        width: el.width, height: el.height, fill: el.polaroid ? 'transparent' : '#ffffff',
        stroke: el.polaroid ? 'transparent' : '#d5dae1',
        strokeWidth: el.polaroid ? 0 : 1.2,
        dash: el.polaroid ? undefined : [10, 8],
    ...
    function konvaBuildText(layer, el, draggable) {
      ...
      el.lines.forEach((line, i) => {
        ...
        const t = new Konva.Text({
          x: 0, y: i * el.lineHeight - el.fontSize * 0.84,
          text: line,
    ...
    function buildKonvaSceneParams() {
      ...
      return {
        ...
        baseImageScale: selectedPhotoPrintSize?.baseImageScale ?? getProductBaseImageScale(state.product),
        baseImageX: selectedPhotoPrintSize?.baseImageX ?? 500,
        baseImageY: selectedPhotoPrintSize?.baseImageY ?? 625,
    ...
    function getTemplatePhotoLayout(
      ...
      const psOverride = (konvaConfig?.printSizeOverrides && konvaConfig.printSizeOverrides[photoPrintSize?.id]) || {};
      const photoScale = psOverride.photoScale ?? pd.scale ?? 1;
      const scaledWidth = exportWidth * photoScale;
      const scaledHeight = exportHeight * photoScale;
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function createCheckoutCompositePreviewUrl(options = {}) {
      ...
      const standardUploadPreviewUrl = hasStandardUpload
        ...
        : '';
      const baseScale = selectedPhotoPrintSize?.baseImageScale ?? 1.12;
      const baseX = selectedPhotoPrintSize?.baseImageX ?? 500;
      const baseY = selectedPhotoPrintSize?.baseImageY ?? 625;
      const baseGroup = `
...
function renderLeftSummary() {
    ...
    async function drawCheckoutBaseImageToCanvas(ctx) {
      ...
      const baseScale = selectedPhotoPrintSize?.baseImageScale ?? ((transformValues.scaleX || 1.12) + (transformValues.scaleY || 1.12)) / 2;
      const baseX = selectedPhotoPrintSize?.baseImageX ?? 500;
      const baseY = selectedPhotoPrintSize?.baseImageY ?? 625;
      let rendered = false;
      ctx.save();
    ...
    function syncGarmentScale() {
      ...
      console.log('[syncGarmentScale]', 'step:', state.step, 'template:', getSelectedTemplate()?.kind, 'focusConfig:', previewFocusConfig, 'garment size:', garment.offsetWidth, 'x', garment.offsetHeight);
    ...
    function renderVisualizer() {
      ...
      {
        const gwrap = document.getElementById('garmentWrap');
        const konvaCanvas = gwrap?.querySelector('canvas');
        const konvaContainer = document.getElementById('konvaPreviewContainer');
        console.log('[renderVisualizer]', 'template:', selectedTemplate?.kind, 'step:', state.step);
        console.log('  garment:', garment.offsetWidth, 'x', garment.offsetHeight);
        console.log('  baseImage:', baseImage?.offsetWidth, 'x', baseImage?.offsetHeight, 'naturalWidth:', baseImage?.naturalWidth, 'naturalHeight:', baseImage?.naturalHeight);
        console.log('  konvaCanvas:', konvaCanvas?.offsetWidth, 'x', konvaCanvas?.offsetHeight, 'style.width:', konvaCanvas?.style.width, 'style.height:', konvaCanvas?.style.height);
        console.log('  konvaContainer:', konvaContainer?.offsetWidth, 'x', konvaContainer?.offsetHeight);
        console.log('  designLayer display:', document.getElementById('designLayer')?.style.display);
        // Check all direct children of garmentWrap for unexpected sizing
        if (gwrap) {
          Array.from(gwrap.children).forEach((child) => {
            const cs = window.getComputedStyle(child);
            console.log('  child:', child.id || child.className || child.tagName, child.offsetWidth, 'x', child.offsetHeight, 'bg:', cs.backgroundColor, 'display:', cs.display);
          });
        }
      }
