...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function konvaUpdateDebugPanel(node) {
      ...
      // Config textScaleMultiplier (baked into fontSize, not into node scale)
      ...
      const currentPsId = state.printSize || '';
      const psOvr = (tplConfig?.printSizeOverrides && tplConfig.printSizeOverrides[currentPsId]) || {};
      const configMul = psOvr.textScaleMultiplier || tplConfig?.textScaleMultiplier || 1;
...
function renderLeftSummary() {
    ...
    function renderVisualizer() {
      ...
      const currentScale = getCurrentPrintScale(selectedPrintSize?.id, selectedTemplate);
      const selectedPhotoPrintSize = getSelectedPhotoPrintSize(undefined, selectedTemplate);
      const productBaseImageScale = selectedPhotoPrintSize?.baseImageScale ?? getProductBaseImageScale(selectedProduct?.id);
