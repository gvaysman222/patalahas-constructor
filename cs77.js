...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function konvaEnsurePreviewStage() {
      ...
      // Find or create container div — direct child of garmentWrap, outside .garment
      let div = mount.querySelector('#' + KONVA_CONTAINER_ID);
      if (!div) {
        div = document.createElement('div');
        div.id = KONVA_CONTAINER_ID;
        mount.appendChild(div);
        // Diagnostic: verify DOM events reach the container
        div.addEventListener('pointerdown', (e) => {
          console.log('[Konva] pointerdown on container div at', e.clientX, e.clientY);
        });
      }
      // Minimal inline styles — position over garmentWrap, let Konva handle the rest
      ...
      if (!konvaPreviewStage) {
        ...
        // Diagnostic: verify Konva stage events
        konvaPreviewStage.on('pointerdown mousedown click', (e) => {
          console.log('[Konva] stage event:', e.type, 'target:', e.target?.name?.() || e.target?.getClassName?.());
        });
      } else {
        // Re-attach if container was re-created
      ...
      // Scale layer so 1000x1250 artboard maps to container pixels
    ...
    async function konvaRenderPreview(scene, sceneKey) {
      ...
      if (!stage || !konvaPreviewLayer) {
        console.warn('[Konva] cannot create stage');
        return;
      }
      console.log('[Konva] rendering preview, elements:', scene.elements.length, 'size:', stage.width() + 'x' + stage.height());
      ...
      console.log('[Konva] done, nodes:', konvaPreviewLayer.getChildren().length);
    }
