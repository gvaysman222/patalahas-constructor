...
(function initConstructorCheckoutTemplateConfig(global) {
  ...
  const photoDiagonalTextPrintSizes = [
    {
      ...
      previewTransformOrigin: 'left top',
      exportAnchor: 'start',
    },
  ];
  ...
  const centerEmbroideryFonts = [
    ...
    },
    {
      ...
      label: 'Шрифт 2',
      family: '"Gooseberry Normal", Georgia, "Times New Roman", serif',
      previewScale: 0.8,
      exportScale: 0.8,
  ...
  global.CONSTRUCTOR_CHECKOUT_TEMPLATE_CONFIG = {
    ...
    fontFamilies: {
      ...
      'emb-center-emb-font-2': '"Gooseberry Normal", Georgia, "Times New Roman", serif',
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const PHOTO_DIAGONAL_TEXT_ROTATION = -25;
    const PHOTO_DIAGONAL_TEXT_X_RATIO = 0.72;
    const PHOTO_DIAGONAL_TEXT_PREVIEW_TOP_RATIO = 0.69;
    const PHOTO_DIAGONAL_TEXT_EXPORT_BASELINE_RATIO = 0.78;
    ...
    function templateSupportsText(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return ['emb-center', 'emb-left', 'photo-top-text', 'photo-bottom-text', 'photo-diagonal-text', 'polaroid-top', 'polaroid-bottom', 'polaroid-tilt'].includes(template.kind);
    }
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function createCheckoutCompositePreviewUrl(options = {}) {
      ...
      if (state.selectedCustom) {
        ...
        const polaroidExportFontScale = getExportFontScale(state.font, selectedTemplate);
        const polaroidTextFontSize = ((selectedPrintSize?.id === 's' ? 36 : 40) * polaroidExportFontScale);
        const polaroidTextLineHeight = ((selectedPrintSize?.id === 's' ? 42 : 48) * polaroidExportFontScale);
        const polaroidBody = `
          <g transform="translate(500 587.5) scale(${currentScale}) rotate(${tilt}) translate(-500 -587.5)">
            ${textTop ? renderSvgTextBlock(textLines, {
              x: 500,
              y: 215,
              lineHeight: polaroidTextLineHeight,
              fontSize: polaroidTextFontSize,
    ...
    async function renderCheckoutCompositePreviewCanvas(canvas) {
      ...
      if (isPolaroidTemplate) {
        ...
        const polaroidPreviewFontScale = getPreviewFontScale(state.font, selectedTemplate);
        const textFontSize = ((selectedPrintSize?.id === 's' ? 11 : 12) * artboardScale) * polaroidPreviewFontScale;
        const textLineHeight = textFontSize * (Number(textRules.lineHeight) || 1.15);
        ...
        if (textTop) {
          drawCanvasTextBlock(ctx, textLines, {
            ...
            fontFamily,
            fontWeight: textRules.fontWeight,
            letterSpacing: getPreviewTextLetterSpacingValue(selectedTemplate, selectedPrintSize, textFontSize),
          });
        }
    ...
    async function getCheckoutCompositePreviewUrl(options = {}) {
      ...
      let previewUrl = '';
      if (isPhotoTextTemplate(selectedTemplate)) {
...
function renderLeftSummary() {
    ...
    function getDesignLayerRenderState() {
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        html = `
          <div class="polaroid-wrap" style="--print-scale:${currentScale}; --text-color:${state.textColor}; --tilt:${textTilt ? '-8deg' : '0deg'}; font-family:${previewFontFamily};">
            ${textTop ? `<div class="polaroid-text" style="max-width:132px; font-size:${previewTextStyle.fontSize}; line-height:${previewTextStyle.lineHeight}; font-weight:${previewTextStyle.fontWeight}; letter-spacing:${previewTextStyle.letterSpacing}; text-transform:${previewTextStyle.textTransform}; -webkit-line-clamp:${previewTextRules.maxLines};">${previewTextHtml}</div>` : ''}
            <div class="polaroid-frame">
              <div class="polaroid-photo">${uploadsHtml}</div>
              <div class="polaroid-gap"></div>
            </div>
            ${textBottom || textTilt ? `<div class="polaroid-text" style="margin-top:8px; max-width:132px; font-size:${previewTextStyle.fontSize}; line-height:${previewTextStyle.lineHeight}; font-weight:${previewTextStyle.fontWeight}; letter-spacing:${previewTextStyle.letterSpacing}; text-transform:${previewTextStyle.textTransform}; -webkit-line-clamp:${previewTextRules.maxLines};">${previewTextHtml}</div>` : ''}
          </div>
        `;
