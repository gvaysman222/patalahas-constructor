  ...
  global.CONSTRUCTOR_CHECKOUT_TEMPLATE_CONFIG = {
    ...
    templateTextConfigMap: {
      ...
      'emb-center': {
        ...
        previewTransform: 'translateX(-50%) scale(var(--print-scale, 1))',
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function getPreviewTextRules(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template)) {
      ...
      if (config && printSize) {
        return {
          ...
          previewTransform: printSize.previewTransform || config.previewTransform || 'translateX(-50%) scale(var(--print-scale, 1))',
      ...
      return {
        ...
        exportCenterY: template?.kind === 'emb-left' ? 350 : 485,
    ...
    function getPreviewTextBlockStyleConfig(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1) {
      ...
      return {
        ...
        transform: rules.previewTransform || 'translateX(-50%) scale(var(--print-scale, 1))',
        textAlign: rules.previewTextAlign || 'center',
        maxWidth: `${Math.round(rules.previewMaxWidth * previewWidthScale)}px`,
        ...
        fontSize: `${(Math.round(basePreviewFontSize * previewFontScale * 100) / 100)}px`,
      };
    }
...
function renderLeftSummary() {
    ...
    function getGarmentPreviewFocusConfig() {
      const template = getSelectedTemplate();
      ...
      return {
        scale: isCompactViewport ? 1.16 : 1.26,
        origin: '50% 42%',
      };
    }
    ...
    function syncGarmentScale() {
      ...
      garment.style.transform = `scale(${previewFocusConfig.scale})`;
      garment.style.transformOrigin = previewFocusConfig.origin;
      garment.style.willChange = 'transform';
    }
