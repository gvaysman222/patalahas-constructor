...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    async function konvaBuildSceneNodes(layer, scene, devMode) {
      ...
      // Design elements
      const isTemplateSelectionStep = state.step === 2;
      for (const el of scene.elements) {
        // On step 2 (template selection), skip frame and placeholder to keep garment fully visible
        if (isTemplateSelectionStep && (el.type === 'frame' || el.type === 'photo-placeholder')) continue;
...
function renderLeftSummary() {
    ...
    function getDesignLayerRenderState() {
      ...
      const selectedPrintSize = getSelectedPrintSize(undefined, selectedTemplate);
      const currentScale = getCurrentPrintScale(selectedPrintSize?.id, selectedTemplate);
      const isPolaroid = state.step > 2 && isPolaroidPhotoTemplate(selectedTemplate);
    ...
    function renderVisualizer() {
      ...
      {
        const cs = window.getComputedStyle(garment);
        const bis = baseImage ? window.getComputedStyle(baseImage) : {};
        const gwrap = document.getElementById('garmentWrap');
        const gws = gwrap ? window.getComputedStyle(gwrap) : {};
        console.log('[renderVisualizer]', 'template:', selectedTemplate?.kind, 'printScale:', currentScale, 'baseImageScale:', productBaseImageScale, 'printSize:', selectedPrintSize?.id);
        console.log('  garment:', garment.offsetWidth, 'x', garment.offsetHeight, 'transform:', cs.transform, 'inset:', cs.inset, 'left:', cs.left, 'top:', cs.top, 'width:', cs.width, 'height:', cs.height);
        console.log('  baseImage:', baseImage?.offsetWidth, 'x', baseImage?.offsetHeight, 'w:', bis.width, 'h:', bis.height);
        console.log('  garmentWrap:', gwrap?.offsetWidth, 'x', gwrap?.offsetHeight, 'w:', gws.width, 'h:', gws.height);
      }
