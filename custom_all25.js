...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function getSimplePhotoLayout(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template)) {
      if (!isSimplePhotoTemplate(template) || !printSize) return null;
      const exportWidth = Number(printSize.exportWidth) || 170;
      const exportHeight = Number(printSize.exportHeight) || 214;
      const exportTop = Number(printSize.exportTop);
      const resolvedExportTop = Number.isFinite(exportTop) ? exportTop : 550;
      const previewScaleX = GARMENT_ARTBOARD_WIDTH / CHECKOUT_EXPORT_ARTBOARD_WIDTH;
      const previewScaleY = GARMENT_ARTBOARD_HEIGHT / CHECKOUT_EXPORT_ARTBOARD_HEIGHT;
      const previewWidth = exportWidth * previewScaleX;
      const previewHeight = exportHeight * previewScaleY;
      return {
        exportX: (CHECKOUT_EXPORT_ARTBOARD_WIDTH - exportWidth) / 2,
        exportY: resolvedExportTop,
        exportWidth,
        exportHeight,
        previewX: (GARMENT_ARTBOARD_WIDTH - previewWidth) / 2,
        previewY: resolvedExportTop * previewScaleY,
        previewWidth,
        previewHeight,
      };
    }
    ...
    function templateSupportsText(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return ['emb-center', 'emb-left', 'polaroid-top', 'polaroid-bottom', 'polaroid-tilt'].includes(template.kind);
    }
    ...
    function isStepComplete(stepId) {
      ...
      if (stepId === 4) {
        const template = getSelectedTemplate();
        if (isSimplePhotoTemplate(template)) return Boolean(getSelectedPrintSize(undefined, template));
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function createCheckoutCompositePreviewUrl(options = {}) {
      ...
      const selectedTemplate = getSelectedTemplate();
      const selectedPrintSize = getSelectedPrintSize(undefined, selectedTemplate);
      const simplePhotoLayout = getSimplePhotoLayout(selectedTemplate, selectedPrintSize);
      ...
      const standardUploadPreviewUrl = hasStandardUpload
        ? await createTemplatePhotoDataUrl(
          simplePhotoLayout?.exportWidth || standardPhotoWidth,
          simplePhotoLayout?.exportHeight || standardPhotoHeight
        )
        : '';
      const fontFamily = getFontFamily(state.font);
      ...
      if (state.selectedCustom) {
        ...
        if (simplePhotoLayout) {
          designMarkup = standardUploadPreviewUrl ? `
            <image
              href="${escapeSvg(standardUploadPreviewUrl)}"
              x="${simplePhotoLayout.exportX}"
              y="${simplePhotoLayout.exportY}"
              width="${simplePhotoLayout.exportWidth}"
              height="${simplePhotoLayout.exportHeight}"
      ...
      if (simplePhotoLayout) {
        if (hasStandardUpload) {
          await drawCurrentUploadToCanvas(
            ctx,
            simplePhotoLayout.exportX,
            simplePhotoLayout.exportY,
            simplePhotoLayout.exportWidth,
            simplePhotoLayout.exportHeight
          );
        }
        ctx.restore();
        return true;
          ...
          <div class="checkout-review-grid">
            <div class="info-card">
              ...
              <div class="stack" style="margin-top:16px; font-size:14px;">
                ...
                ${!isCustomSelected() && templateSupportsText() ? rowHtml('Нанесение', getApplicationTypeLabel() || 'Печать') : ''}
                ...
                ${templateSupportsText() ? rowHtml('Надпись вышивкой', state.isEmbroideryText ? `Да, ${getEmbroiderySymbolsCount()} симв.` : 'Нет') : ''}
              </div>
            </div>
...
function renderLeftSummary() {
    ...
    function getDesignLayerRenderState() {
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        `;
      } else if (!hideDesign && simplePhotoLayout) {
        html = activeUpload ? `
          <div
            class="image-box"
            style="left:${simplePhotoLayout.previewX}px; top:${simplePhotoLayout.previewY}px; width:${simplePhotoLayout.previewWidth}px; height:${simplePhotoLayout.previewHeight}px; border:none; border-radius:0; background:transparent;"
          >
            ${uploadsHtml}
          </div>
        ` : '';
