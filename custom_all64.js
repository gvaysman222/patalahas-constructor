...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const POLAROID_FRAME_INNER_X_RATIO = 0.055;
    const POLAROID_FRAME_INNER_Y_RATIO = 0.045;
    const POLAROID_FRAME_INNER_WIDTH_RATIO = 0.89;
    const POLAROID_FRAME_INNER_HEIGHT_RATIO = 0.74;
    ...
    const POLAROID_INSIDE_TEXT_X_RATIO = 0.11;
    const POLAROID_INSIDE_TEXT_Y_RATIO = 0.79;
    const POLAROID_INSIDE_TEXT_WIDTH_RATIO = 0.78;
    const POLAROID_INSIDE_TEXT_HEIGHT_RATIO = 0.145;
    const POLAROID_INSIDE_TEXT_PADDING_X_RATIO = 0.05;
    const POLAROID_INSIDE_TEXT_PADDING_Y_RATIO = 0.06;
      ...
      if (config) {
        ...
        if (isPolaroidInsideTextTemplate(template)) {
          ...
          if (insideLayout) {
            ...
            const topInset = Math.max(0, (insideLayout.exportInnerHeight - contentHeight) / 2);
            return {
              x: insideLayout.exportRightX,
              anchor: 'end',
              fontSize,
              lineHeight,
              y: insideLayout.exportInnerY + topInset + (fontSize * POLAROID_INSIDE_TEXT_BASELINE_RATIO),
            };
          }
        }
