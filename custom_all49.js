...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const POLAROID_INSIDE_TEXT_Y_RATIO = 0.812;
    const POLAROID_INSIDE_TEXT_WIDTH_RATIO = 0.74;
    const POLAROID_INSIDE_TEXT_HEIGHT_RATIO = 0.105;
    ...
    function getProductCardNote(item) {
      ...
      if (normalizedNote.includes('наруж')) return 'Швы наружу';
      if (normalizedNote.includes('классич')) return 'Швы внутри';
      return displayNote;
    }
      ...
      if (config) {
        ...
        if (isPolaroidInsideTextTemplate(template)) {
          ...
          if (insideLayout) {
            ...
            const topInset = Math.max(0, (insideLayout.exportHeight - contentHeight) / 2);
            return {
              x: insideLayout.exportCenterX,
              anchor: 'middle',
              fontSize,
              lineHeight,
              y: insideLayout.exportY + topInset + (fontSize * POLAROID_INSIDE_TEXT_BASELINE_RATIO),
            };
          }
        }
    ...
    function buildTildaCartTitle(payload = getOrderPayload()) {
      ...
      const parts = [
        ...
        !isCustom && payload.hasTextCustomization && payload.textColor ? `Цвет текста ${String(payload.textColor).replace('#', '')}` : '',
    ...
    function updateOrderFormFields() {
      ...
      const summaryLines = [
        ...
        payload.hasTextCustomization && payload.textColor ? `Цвет текста: ${payload.textColor}` : '',
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function renderCheckoutCompositePreviewCanvas(canvas) {
      ...
      if (domMetrics && !isPhotoTextTemplate(selectedTemplate)) {
    ...
    async function getCheckoutCompositePreviewUrl(options = {}) {
      ...
      let previewUrl = '';
      if (isPhotoTextTemplate(selectedTemplate) || isPolaroidPhotoTemplate(selectedTemplate)) {
        const canvasPreview = await createCheckoutCompositePreviewFileFromCanvas('image/png');
...
// ===== constructor-checkout-mvp-tilda-js-2-steps.html =====
    ...
    function renderStepContent() {
      ...
      if (state.step === 4) {
              ...
              <div>
                ...
                <div class="size-dropdown" data-step-dropdown>
                  <button class="dropdown-toggle" data-dropdown-toggle type="button" aria-expanded="false">
                    <span class="font-dropdown-label" id="photoPrintSizeDropdownLabel">${getPhotoPrintSizeLabel(selectedPhotoPrintSize)}</span>
                  </button>
                  <div class="dropdown-panel">
                    <div class="dropdown-panel-inner">
                      ${availablePhotoPrintSizes.map((p) => `
                        <button class="wide-btn ${selectedPhotoPrintSize?.id === p.id ? 'active' : ''}" data-photo-print-size="${p.id}" type="button">
                          <span>${p.name}</span>
                        </button>
                      `).join('')}
                    </div>
        ...
        const showTextTemplateSettings = templateSupportsText();
        ...
        const selectedFontFamily = escapeAttr(getFontFamily(state.font));
          ...
          <div class="section">
            ...
            <div>
              ...
              <div class="font-dropdown" data-step-dropdown>
                <button class="dropdown-toggle" data-dropdown-toggle type="button" aria-expanded="false">
                  <span class="font-dropdown-label" id="fontDropdownLabel" style="font-family:${selectedFontFamily};">${getFontDisplayName(state.font)}</span>
                </button>
                <div class="dropdown-panel">
                  <div class="dropdown-panel-inner">
                    ${availableFonts.map((fontOption) => `
                      <button class="font-btn font-option-btn ${state.font === fontOption.id ? 'active' : ''}" data-font-value="${escapeAttr(fontOption.id)}" style="font-family:${escapeAttr(getFontFamily(fontOption.id))};" type="button">
...
function renderLeftSummary() {
    ...
    function drawCanvasTextElementInBox(ctx, element, box, domMetrics) {
      ...
      if (!lines.some((line) => line.length > 0)) {
        ...
        ctx.save();
        ctx.font = getCanvasFont(fontSize, fontFamily, fontWeight);
        lines = splitCanvasTextByWidth(ctx, text, Math.max(1, box.width), maxLines, letterSpacing);
        ctx.restore();
      }
    ...
    function updateStepFourControls(options = {}) {
      ...
      const photoSizeDropdownLabel = container.querySelector('#photoPrintSizeDropdownLabel');
      if (photoSizeDropdownLabel) photoSizeDropdownLabel.textContent = getPhotoPrintSizeLabel(selectedPhotoPrintSize);
    ...
    function getDesignLayerRenderState() {
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        const insideTextBlockHtml = polaroidInsideTextLayout ? `
          <div
            style="position:absolute; left:${polaroidInsideTextLayout.localLeft}; top:${polaroidInsideTextLayout.localTop}; width:${polaroidInsideTextLayout.localWidth}; height:${polaroidInsideTextLayout.localHeight}; overflow:hidden; pointer-events:none; display:flex; align-items:center; justify-content:center;"
          >
            <div style="width:100%; max-width:none; color:${state.textColor}; text-align:center; line-height:${Math.round(((Number(polaroidInsideTextMetrics?.lineHeight || 0) / Math.max(1, Number(polaroidInsideTextMetrics?.fontSize || 1))) * 1000)) / 1000}; font-weight:${previewTextStyle.fontWeight}; letter-spacing:${previewTextStyle.letterSpacing}; text-transform:${previewTextStyle.textTransform}; font-size:${Math.round(((Number(polaroidInsideTextMetrics?.fontSize || 0) / Math.max(1, Number(polaroidInsideTextLayout.exportHeight || 1))) * 10000)) / 100}%; font-family:${previewFontFamily}; display:-webkit-box; -webkit-box-orient:vertical; -webkit-line-clamp:${previewTextRules.maxLines}; overflow:hidden;">
              ${previewTextHtml}
            </div>
          </div>
        ` : '';
