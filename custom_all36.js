...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function isPhotoTextTemplate(template = getSelectedTemplate()) {
      return isPhotoTopTextTemplate(template) || isPhotoBottomTextTemplate(template);
    }
    ...
    function getGarmentPreviewFocusConfig(template = getSelectedTemplate()) {
      ...
      if (isPhotoTextTemplate(template)) {
        return {
          ...
          focusX: 0.5,
          focusY: isPhotoBottomTextTemplate(template) ? 0.49 : 0.45,
        };
      }
    ...
    function getPreviewTextBlockStyleConfig(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1) {
      ...
      return {
        left: rules.previewLeft || '50%',
        right: rules.previewRight || 'auto',
        width: rules.previewWidth || 'min(80%, var(--preview-text-max-width, 190px))',
        transform: transformParts.join(' '),
        transformOrigin: rules.previewTransformOrigin || 'center top',
        textAlign: rules.previewTextAlign || 'center',
    ...
    function templateSupportsText(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return ['emb-center', 'emb-left', 'photo-top-text', 'photo-bottom-text', 'polaroid-top', 'polaroid-bottom', 'polaroid-tilt'].includes(template.kind);
    }
    ...
    const isEmbroideryAvailable = (template = getSelectedTemplate()) => templateSupportsText(template);
      ...
      if (state.selectedCustom) {
        ...
        if (isPhotoTextTemplate(selectedTemplate) && templatePhotoLayout) {
          ...
          const photoMarkup = standardUploadPreviewUrl ? `
            ...
            />
          ` : '';
          designMarkup = isPhotoBottomTextTemplate(selectedTemplate)
          ...
          <div class="checkout-review-grid">
            <div class="info-card">
              ...
              <div class="stack" style="margin-top:16px; font-size:14px;">
                ...
                ${templateSupportsText(selectedTemplate) ? rowHtml('Надпись вышивкой', state.isEmbroideryText ? `Да, ${getEmbroiderySymbolsCount()} симв.` : 'Нет') : ''}
              </div>
            </div>
...
function renderLeftSummary() {
    ...
    function getDesignLayerRenderState() {
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        ` : '';
        html = isPhotoBottomTextTemplate(selectedTemplate)
