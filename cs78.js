...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function konvaBuildText(layer, el, draggable) {
      ...
      // Measure approximate total height for hit area
      const totalH = el.fontSize + (el.lines.length - 1) * el.lineHeight;
      const textW = 800;
      const hitW = Math.min(textW, 400); // reasonable hit width

      // Hit area rect — transparent but listening, makes the Group draggable
      let hitX = 0;
      if (align === 'center') hitX = -hitW / 2;
      else if (align === 'right') hitX = -hitW;
      textGroup.add(new Konva.Rect({
        x: hitX, y: -el.fontSize,
        width: hitW, height: totalH + el.fontSize,
        fill: 'transparent', listening: true, // ← hit area
      }));
