...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function setupPreviewZoom() {
      ...
      const applyTransform = (originX, originY, zoom, panX, panY, animate) => {
        garmentWrap.style.transition = animate ? 'transform .25s ease' : 'none';
        garmentWrap.style.transformOrigin = `${originX}% ${originY}%`;
        garmentWrap.style.transform = zoom > 1
      ...
      const resetTransform = (animate) => {
        applyTransform(50, 50, 1, 0, 0, animate);
      };
      ...
      if (isDesktop) {
        garmentWrap.addEventListener('mouseenter', (e) => {
          ...
          applyTransform(ox, oy, DESKTOP_ZOOM, 0, 0, false);
        }, { signal });

        garmentWrap.addEventListener('mousemove', (e) => {
          if (currentZoom <= 1) return;
          const rect = garmentWrap.parentElement.getBoundingClientRect();
          const ox = ((e.clientX - rect.left) / rect.width) * 100;
          const oy = ((e.clientY - rect.top) / rect.height) * 100;
          garmentWrap.style.transition = 'none';
