...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function showCartUploadOverlay(cartWindow) {
      if (!cartWindow) return;
      let overlay = cartWindow.querySelector('.cart-upload-overlay');
      if (!overlay) {
        overlay = document.createElement('div');
        overlay.className = 'cart-upload-overlay';
        overlay.innerHTML = `
        ...
        style.textContent = `
          .cart-upload-overlay { position: absolute; inset: 0; z-index: 9999; display: flex; align-items: center; justify-content: center; pointer-events: all; }
          .cart-upload-overlay-backdrop { position: absolute; inset: 0; background: rgba(255,255,255,.82); }
          .cart-upload-overlay-spinner { position: relative; display: flex; flex-direction: column; align-items: center; gap: 10px; font-size: 14px; font-weight: 600; color: #111; }
        `;
        overlay.prepend(style);
        cartWindow.style.position = cartWindow.style.position || 'relative';
        cartWindow.appendChild(overlay);
      }
      overlay.style.display = 'flex';
      cartWindow.querySelectorAll('input, textarea, select, button[type="submit"], .t-submit').forEach((el) => {
        el.dataset.cartUploadDisabled = el.disabled ? 'was-disabled' : 'enabled';
        el.disabled = true;
      });
    }
    function hideCartUploadOverlay(cartWindow) {
      if (!cartWindow) return;
      const overlay = cartWindow.querySelector('.cart-upload-overlay');
      if (overlay) overlay.style.display = 'none';
      cartWindow.querySelectorAll('[data-cart-upload-disabled]').forEach((el) => {
        if (el.dataset.cartUploadDisabled !== 'was-disabled') {
          el.disabled = false;
        }
        delete el.dataset.cartUploadDisabled;
      });
    }
    async function submitOrderForm(event) {
      ...
      try {
        ...
        if (openedTildaForm) {
          showCartUploadOverlay(openedCartWindow);
          try {
            await delay(250);
            ...
            hideCartUploadOverlay(openedCartWindow);
          }
        }
