...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const extras = [
      { id: 'hand-drawn', name: 'Рисованные элементы на фотографии', description: 'К любому шаблону с фотографией вы можете добавить рисованные элементы поверх фото.', price: 350 },
      { id: 'polaroid-photo', name: 'Полароид', description: 'Ваши фотографии будут напечатаны на оригинальных картриджах Fujifilm', price: 350 },
      { id: 'cassette', name: 'Аудио открытка «кассета»', description: 'На кассету можно записать своё 60-секундное послание, которое получатель сможет прослушать.', price: 350 },
    ];
    // количество допов
    const extrasCount = {};
    ...
    const KONVA_TEMPLATE_CONFIGS = {
      ...
      'photo-diagonal-text': {
        ...
        printSizeOverrides: {
          l: { photoExportX: 436, photoExportY: 483, photoScale: 0.822 },
          m: { exportCenterY: 493, textScaleMultiplier: 1.503 },
          s: { diagonalExportY: 592.4 },
          's:s': { photoExportY: 483, diagonalExportY: 592.4 },
          'm:s': { photoExportY: 483 },
          's:l': { photoExportX: 436, photoExportY: 483, photoScale: 0.822, diagonalExportY: 592.4 },
      ...
      'polaroid-diagonal-text': {
        ...
        diagonalText: { rotation: -24, xRatio: 0.87, previewTopRatio: 0.81, exportBaselineRatio: 0.9, exportY: 592.9 },
    ...
    function konvaResolveScene(params) {
      ...
      if (config.hasText && textLines.length > 0) {
        ...
        if (config.textPosition === 'diagonal-photo' && config.diagonalText) {
          ...
          const overrideY = comboOverride.diagonalExportY ?? psOverride.diagonalExportY ?? dt.exportY ?? null;
          const textY = overrideY != null ? overrideY : ref.y + ref.height * dt.exportBaselineRatio;
          textLayout = { x: ref.x + ref.width * dt.xRatio, y: textY, fontSize, lineHeight, anchor: 'middle', rotation: dt.rotation, clipRect: null };
    ...
    function getProductionModeStatusLabel(snapshotState = state) {
      ...
      if (productionMode === 'fast') return '✓ До 3 рабочих дней';
      return 'Стандартное производство';
    }
    function getProductionModeSummaryLabel(snapshotState = state) {
      const productionMode = getProductionMode(snapshotState);
      if (productionMode === 'superfast') return 'Производство за 24 часа';
      if (productionMode === 'fast') return 'Ускоренное производство';
      return 'Стандартное производство';
    }
    ...
    const syncTextColorToGarment = () => {
      const iznankaProducts = ['tee-iznanka', 'longsleeve-choice', 'sweatshirt-iznanka'];
      const graphiteColors = ['Графит', 'Графитовый'];
      const darkColors = ['Чёрный', ...graphiteColors];
      if (iznankaProducts.includes(state.product)) {
        if (graphiteColors.includes(state.color)) state.textColor = 'rgb(255, 255, 255)';
        return;
      }
    ...
    function getPhotoDiagonalTextLayout(
      ...
      const diagonalYOverride = psOverride.diagonalExportY ?? konvaConfig?.diagonalText?.exportY ?? null;
    ...
    function updateOrderFormFields() {
      ...
      const summaryLines = [
        ...
        `Тип производства: ${getProductionModeSummaryLabel(payload)}`,
                ...
                return `
                  <div class="extra-card ${count > 0 ? 'active' : ''}">
                    <div style="text-align:left;">
                      ...
                      <div class="subtext">${e.description || 'Можно добавить к покупке'}</div>
                    </div>
              ...
              <div class="stack production-options" style="margin-top:16px;">
                <button class="toggle-card production-option-card ${productionMode === 'standard' ? 'active' : ''}" data-production-mode="standard" type="button" aria-pressed="${productionMode === 'standard'}">
                  <div class="production-option-main">
                    <div class="production-option-title">Стандартное</div>
                    <div class="subtext">Срок изготовления: до 7 рабочих дней</div>
                  </div>
                </button>
                ...
                <button class="toggle-card production-option-card ${productionMode === 'fast' ? 'active' : ''}" data-production-mode="fast" type="button" aria-pressed="${productionMode === 'fast'}">
                  <div class="production-option-main">
                    <div class="production-option-title">Ускоренное</div>
                    <div class="subtext">до 3 рабочих дней</div>
                  </div>
                ...
                <button class="toggle-card production-option-card ${productionMode === 'superfast' ? 'active' : ''}" data-production-mode="superfast" type="button" aria-pressed="${productionMode === 'superfast'}">
                  <div class="production-option-main">
                    <div class="production-option-title">Ускоренное</div>
                    <div class="subtext">Сделать за 24 часа</div>
                  </div>
