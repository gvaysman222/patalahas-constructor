...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function renderTemplateChoiceCard(item, options = {}) {
      ...
      const hasSizePrices = mode === 'template' && item.prices;
      const priceLabel = (mode === 'custom' || hasSizePrices) ? `от ${formatPrice(item.price)}` : formatPrice(item.price);
      const priceMark = (mode === 'custom' || hasSizePrices) ? '' : '+';
      return `
    ...
    function showCartUploadOverlay() {
      ...
      if (!overlay) {
        ...
        style.textContent = `
          #cartUploadOverlay { position: fixed; inset: 0; z-index: 99999; display: flex; align-items: center; justify-content: center; }
          .cart-upload-overlay-backdrop { position: absolute; inset: 0; background: rgba(255,255,255,.35); }
          .cart-upload-overlay-spinner { position: relative; display: flex; flex-direction: column; align-items: center; gap: 10px; font-size: 14px; font-weight: 600; color: #111; background: rgba(255,255,255,.92); padding: 24px 32px; border-radius: 16px; box-shadow: 0 4px 24px rgba(0,0,0,.10); }
        `;
