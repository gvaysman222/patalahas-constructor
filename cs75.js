...
(function initConstructorCheckoutTemplateConfig(global) {
  ...
  const polaroidInsidePhotoPrintSizes = [
    {
      id: 's',
      name: '8 x 11,2',
      exportWidth: 110,
  ...
  const photoTopTextPrintSizes = centerTextPrintSizes.map((size) => ({
    ...size,
    maxLines: 1,
  }));
  const photoBottomTextPrintSizes = centerTextPrintSizes.map((size) => ({
    ...size,
    maxLines: 3,
  }));
  const photoDiagonalTextPrintSizes = [
    {
      id: 's',
      name: 'Стандарт',
      heightLabel: '0,8-1 см',
      ...
      previewWidth: 'min(34%, var(--preview-text-max-width, 150px))',
      previewTextAlign: 'center',
      previewTransformOrigin: 'center top',
      exportAnchor: 'middle',
    },
  ];
  const polaroidInsideTextPrintSizes = [
    {
      id: 's',
      name: 'Стандарт',
      heightLabel: '0,25 см',
      ...
      previewTransformOrigin: 'right center',
    },
  ];
  ...
  global.CONSTRUCTOR_CHECKOUT_TEMPLATE_CONFIG = {
    templates: [
      ...
      { id: 'polaroid-bottom-text', name: 'Полароид + надпись снизу', uploads: 1, kind: 'polaroid-bottom-text', price: 950 },
    ...
    templatePhotoConfigMap: {
      ...
      'polaroid-inside-text': {
        sizes: polaroidInsidePhotoPrintSizes,
        defaultPhotoPrintSize: 's',
        exportTop: 430,
      },
    },
    templateTextConfigMap: {
      ...
      'photo-diagonal-text': {
        ...
        fontOptions: centerPrintFonts,
        defaultPrintSize: 's',
        defaultFont: 'emb-center-print-font-1',
      ...
      'polaroid-inside-text': {
        ...sharedTextTemplateBaseConfig,
        printSizes: polaroidInsideTextPrintSizes,
        supportsEmbroidery: false,
        previewMaxWidth: 104,
        previewTop: '50%',
        previewLeft: '50%',
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const POLAROID_FRAME_IMAGE_URL = 'https://www.dropbox.com/scl/fo/04y8ovljbyi645ppvs0xe/AAZUhi-Z6nvGh707riBYscA/rama.png?rlkey=pnn5zy5e82rhap6fbta5ks53v&st=q5sskrnr&dl=0';
    const POLAROID_FRAME_EXPORT_TOP = 430;
    const POLAROID_FRAME_INNER_X_RATIO = 0.0465;
    const POLAROID_FRAME_INNER_Y_RATIO = 0.0332;
    const POLAROID_FRAME_INNER_WIDTH_RATIO = 0.907;
    const POLAROID_FRAME_INNER_HEIGHT_RATIO = 0.8029;
    const POLAROID_TOP_TEXT_PREVIEW_GAP = 14;
    const POLAROID_TOP_TEXT_EXPORT_GAP = 18;
    const POLAROID_BOTTOM_TEXT_PREVIEW_GAP = 14;
    const POLAROID_BOTTOM_TEXT_EXPORT_GAP = 18;
    const POLAROID_DIAGONAL_TEXT_ROTATION = -22;
    const POLAROID_DIAGONAL_TEXT_X_RATIO = 0.84;
    const POLAROID_DIAGONAL_TEXT_PREVIEW_TOP_RATIO = 0.76;
    const POLAROID_DIAGONAL_TEXT_EXPORT_BASELINE_RATIO = 0.84;
    const POLAROID_INSIDE_TEXT_X_RATIO = 0.13;
    const POLAROID_INSIDE_TEXT_Y_RATIO = 0.832;
    const POLAROID_INSIDE_TEXT_WIDTH_RATIO = 0.78;
    const POLAROID_INSIDE_TEXT_HEIGHT_RATIO = 0.14;
    const POLAROID_INSIDE_TEXT_PADDING_X_RATIO = 0.03;
    const POLAROID_INSIDE_TEXT_PADDING_Y_RATIO = 0.045;
    const POLAROID_INSIDE_TEXT_BASELINE_RATIO = 0.84;
    ...
    const KONVA_POLAROID_INNER = { xR: 0.0465, yR: 0.0332, wR: 0.907, hR: 0.8029 };
    ...
    const KONVA_TEMPLATE_CONFIGS = {
      'emb-center': {
        ...
        textPosition: 'center',
        textDefaults: { exportX: 500, exportCenterY: 503, exportCenterYWithUpload: 780, anchor: 'middle' },
      ...
      'emb-left': {
        hasText: true, hasPhoto: false, hasFrame: false, hasStandardUpload: true,
        textPosition: 'center',
        textDefaults: { exportX: 625, exportCenterY: 433, exportCenterYWithUpload: 780, anchor: 'end' },
        standardUpload: { x: 300, y: 363, width: 400, height: 350, radius: 14 },
        layerOrder: ['standardUpload', 'text'],
      },
      'simple-photo': {
      ...
      'photo-top-text': {
        ...
        textDefaults: { exportX: 500, exportCenterY: 500, anchor: 'middle' },
      ...
      'polaroid-top-text': {
        hasText: true, hasPhoto: true, hasFrame: true, isPolaroid: true,
        textPosition: 'above-frame', photoPosition: 'frame-inner',
        frameExportTop: 430,
        textDefaults: { exportX: 500, exportCenterY: 500, anchor: 'middle' },
        textGap: { preview: 14, export: 18 }, textDescentRatio: 0.32,
        layerOrder: ['text', 'photo', 'frame'],
      },
      'polaroid-bottom-text': {
        hasText: true, hasPhoto: true, hasFrame: true, isPolaroid: true,
        textPosition: 'below-frame', photoPosition: 'frame-inner',
        frameExportTop: 430,
        textDefaults: { exportX: 500, exportCenterY: 500, anchor: 'middle' },
        textGap: { preview: 14, export: 18 },
        layerOrder: ['photo', 'frame', 'text'],
      },
      'polaroid-diagonal-text': {
        hasText: true, hasPhoto: true, hasFrame: true, isPolaroid: true,
        textPosition: 'diagonal-frame', photoPosition: 'frame-inner',
        frameExportTop: 430,
        diagonalText: { rotation: -22, xRatio: 0.84, previewTopRatio: 0.76, exportBaselineRatio: 0.84 },
        layerOrder: ['photo', 'frame', 'text'],
      },
      'polaroid-inside-text': {
        hasText: true, hasPhoto: true, hasFrame: true, isPolaroid: true,
        textPosition: 'inside-frame', photoPosition: 'frame-inner',
        frameExportTop: 430,
        insideText: { xRatio: 0.13, yRatio: 0.832, widthRatio: 0.78, heightRatio: 0.14, paddingXRatio: 0.03, paddingYRatio: 0.045, baselineRatio: 0.84 },
        layerOrder: ['photo', 'frame', 'insideText'],
      },
    };
    ...
    function konvaResolveScene(params) {
      const {
        ...
        frameSrc = null, baseImageSrc = null, baseImageScale = KONVA_BASE_SCALE,
        hasUpload = false, isCustom = false, hideDesign = false,
        textExportOverrides = {}, printScale = 1,
      } = params;

      const config = KONVA_TEMPLATE_CONFIGS[templateKind];
      const scene = {
        artboard: { width: KONVA_EXPORT_W, height: KONVA_EXPORT_H },
        baseImage: baseImageSrc ? { src: baseImageSrc, scale: baseImageScale } : null,
        elements: [],
      };
      ...
      const fontSize = exportFontSize * exportFontScale;
      const lineHeight = exportLineHeight * exportFontScale;
      const lineCount = Math.max(1, textLines.length);

      // Resolve frame layout (polaroid)
      let frameLayout = null;
      if (config.hasFrame) {
        const fw = photoExportWidth / KONVA_POLAROID_INNER.wR;
        const fh = photoExportHeight / KONVA_POLAROID_INNER.hR;
        const fx = (KONVA_EXPORT_W - fw) / 2;
        const fy = config.frameExportTop || photoExportTop;
        frameLayout = {
          x: fx, y: fy, width: fw, height: fh,
          photoX: fx + fw * KONVA_POLAROID_INNER.xR,
          photoY: fy + fh * KONVA_POLAROID_INNER.yR,
          photoWidth: photoExportWidth, photoHeight: photoExportHeight,
        };
      }

      // Resolve photo layout (non-frame)
      let photoLayout = null;
      if (config.hasPhoto && !config.hasFrame) {
        const px = (KONVA_EXPORT_W - photoExportWidth) / 2;
        let py = photoExportTop;
        if (config.photoPosition === 'below-text' && config.hasText) {
          const td = { ...config.textDefaults, ...textExportOverrides };
        ...
        photoLayout = { x: px, y: py, width: photoExportWidth, height: photoExportHeight };
      }
      ...
      if (config.hasText && textLines.length > 0) {
        const td = { ...config.textDefaults, ...textExportOverrides };
        if (config.textPosition === 'inside-frame' && config.insideText && frameLayout) {
          const it = config.insideText;
          const ex = frameLayout.x + frameLayout.width * it.xRatio;
          const ey = frameLayout.y + frameLayout.height * it.yRatio;
          const ew = frameLayout.width * it.widthRatio;
          const eh = frameLayout.height * it.heightRatio;
          const padY = eh * it.paddingYRatio;
          const padX = ew * it.paddingXRatio;
          const innerY = ey + padY;
          const innerH = Math.max(1, eh - padY * 2);
          const contentH = fontSize + Math.max(0, lineCount - 1) * lineHeight;
          const fitScale = Math.min(1, innerH / Math.max(1, contentH));
          const ff = fontSize * fitScale, fl = lineHeight * fitScale;
          const fittedH = ff + Math.max(0, lineCount - 1) * fl;
          const topInset = Math.max(0, (innerH - fittedH) / 2);
          textLayout = { x: ex + ew - padX, y: innerY + topInset + ff * it.baselineRatio, fontSize: ff, lineHeight: fl, anchor: 'end', rotation: 0, clipRect: { x: ex, y: ey, width: ew, height: eh } };
        } else if ((config.textPosition === 'diagonal-photo' || config.textPosition === 'diagonal-frame') && config.diagonalText) {
          const dt = config.diagonalText;
          const ref = config.textPosition === 'diagonal-frame' && frameLayout
            ? frameLayout
            : (photoLayout || { x: (KONVA_EXPORT_W - photoExportWidth) / 2, y: photoExportTop, width: photoExportWidth, height: photoExportHeight });
          ...
          textLayout = { x: td.exportX || 500, y: photoLayout.y + photoLayout.height + (config.textGap?.export || 20) + fontSize, fontSize, lineHeight, anchor: td.anchor || 'middle', rotation: 0, clipRect: null };
        } else if (config.textPosition === 'below-frame' && frameLayout) {
          textLayout = { x: td.exportX || 500, y: frameLayout.y + frameLayout.height + (config.textGap?.export || 18) + fontSize, fontSize, lineHeight, anchor: td.anchor || 'middle', rotation: 0, clipRect: null };
        } else if (config.textPosition === 'above-frame' && frameLayout) {
          textLayout = { x: td.exportX || 500, y: frameLayout.y - (config.textGap?.export || 18) - (fontSize * (config.textDescentRatio || 0.32)) - (Math.max(0, lineCount - 1) * lineHeight), fontSize, lineHeight, anchor: td.anchor || 'middle', rotation: 0, clipRect: null };
      ...
      for (const layerName of (config.layerOrder || [])) {
        if (layerName === 'photo' || layerName === 'standardUpload') {
          if (layerName === 'standardUpload' && config.hasStandardUpload && hasUpload && photoSrc) {
            ...
            const isFrameInner = config.hasFrame && frameLayout;
            const lay = isFrameInner ? { x: frameLayout.photoX, y: frameLayout.photoY, width: frameLayout.photoWidth, height: frameLayout.photoHeight } : photoLayout;
            if (lay) {
              if (hasUpload && photoSrc) {
                scene.elements.push({ type: 'user-photo', name: 'photo', x: lay.x, y: lay.y, width: lay.width, height: lay.height, src: photoSrc, crop: photoCrop, radius: 0 });
              } else {
                scene.elements.push({ type: 'photo-placeholder', name: 'photoPlaceholder', x: lay.x, y: lay.y, width: lay.width, height: lay.height, polaroid: Boolean(config.hasFrame) });
              }
            }
          }
        } else if (layerName === 'frame' && frameLayout && frameSrc) {
          scene.elements.push({ type: 'frame', name: 'frame', x: frameLayout.x, y: frameLayout.y, width: frameLayout.width, height: frameLayout.height, src: frameSrc });
    ...
    function konvaUpdateDebugPanel(node) {
      ...
      const w = Math.round((node.width ? node.width() * (node.scaleX ? node.scaleX() : 1) : 0) * 10) / 10;
      const h = Math.round((node.height ? node.height() * (node.scaleY ? node.scaleY() : 1) : 0) * 10) / 10;
        ...
        try {
          ...
          if (img) {
            ...
            const group = new Konva.Group({
              name: 'baseImage',
              x: KONVA_EXPORT_W / 2, y: KONVA_EXPORT_H / 2,
              scaleX: scene.baseImage.scale, scaleY: scene.baseImage.scale,
              offsetX: KONVA_EXPORT_W / 2, offsetY: KONVA_EXPORT_H / 2,
    ...
    async function konvaBuildUserPhoto(layer, el, draggable) {
      ...
      if (el.fill) {
        group.add(new Konva.Rect({ width: el.width, height: el.height, fill: el.fill, cornerRadius: el.radius || 0, listening: false }));
      }
      ...
      if (draggable) {
        group.on('dragmove', () => konvaUpdateDebugPanel(group));
    ...
    function konvaBuildPlaceholder(layer, el, draggable) {
      const group = new Konva.Group({ name: el.name || 'placeholder', x: el.x, y: el.y, draggable, listening: draggable });
      const lines = el.polaroid ? ['ТУТ БУДЕТ', 'ФОТО'] : ['ТУТ БУДЕТ', 'ВАШЕ ФОТО'];
      group.add(new Konva.Rect({
        width: el.width, height: el.height, fill: '#ffffff',
        stroke: el.polaroid ? '#e4e9f0' : '#d5dae1',
        strokeWidth: el.polaroid ? 0.8 : 1.2,
        dash: el.polaroid ? undefined : [10, 8], listening: false,
      }));
      const fs = Math.max(6, Math.min(el.width * 0.09, el.polaroid ? 9 : 10));
      const lh = fs * 1.22;
      ...
      lines.forEach((line, i) => {
        group.add(new Konva.Text({
          ...
          fill: el.polaroid ? '#97a1af' : '#7b8593', align: 'center', letterSpacing: 0.35, listening: false,
        }));
      });
      if (draggable) {
        group.on('dragmove', () => konvaUpdateDebugPanel(group));
    ...
    async function konvaBuildFrame(layer, el, draggable) {
      ...
      const node = new Konva.Image({
        ...
        width: dw, height: dh, draggable, listening: draggable,
      });
      if (draggable) {
        node.on('dragmove', () => konvaUpdateDebugPanel(node));
    ...
    function konvaBuildText(layer, el, draggable) {
      ...
      let container = layer;
      if (hasClip) {
        const clipGroup = new Konva.Group({ clipX: el.clipRect.x, clipY: el.clipRect.y, clipWidth: el.clipRect.width, clipHeight: el.clipRect.height, listening: false });
        layer.add(clipGroup);
        container = clipGroup;
      }
      ...
      el.lines.forEach((line, i) => {
        if (!line) return;
        let tx = 0;
        if (align === 'center') tx = -textW / 2;
        else if (align === 'right') tx = -textW;

        textGroup.add(new Konva.Text({
          x: tx, y: i * el.lineHeight - el.fontSize * 0.84,
          width: textW, text: line,
      ...
      if (draggable) {
        textGroup.on('dragmove', () => konvaUpdateDebugPanel(textGroup));
        textGroup.on('mouseenter', () => { document.body.style.cursor = 'move'; });
        textGroup.on('mouseleave', () => { document.body.style.cursor = ''; });
      }
    ...
    let konvaPreviewContainerId = 'konvaPreviewContainer';
    let konvaPreviewLastKey = '';

    function konvaEnsurePreviewStage(container, w, h) {
      let div = container.querySelector('#' + konvaPreviewContainerId);
      if (!div) {
        div = document.createElement('div');
        div.id = konvaPreviewContainerId;
        div.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;pointer-events:auto;z-index:20;';
        container.appendChild(div);
      }
      // Use higher pixel ratio for crisp text/images in preview
      const minPixelRatio = Math.max(2, window.devicePixelRatio || 1);
      Konva.pixelRatio = minPixelRatio;
      ...
      // In dev mode, the layer and stage must be listening so drag events propagate
      konvaPreviewLayer.listening(KONVA_DEV_MODE);
      konvaPreviewStage.listening(KONVA_DEV_MODE);
      return konvaPreviewStage;
    }

    async function konvaRenderPreview(container, scene, sceneKey) {
      if (!container || !scene) { console.warn('[Konva] render skipped: no container or scene'); return; }
      const rect = container.getBoundingClientRect();
      if (!rect.width || !rect.height) { console.warn('[Konva] render skipped: container has no size', rect); return; }
      if (sceneKey && sceneKey === konvaPreviewLastKey) { return; }
      console.log('[Konva] rendering preview, elements:', scene.elements.length, 'container:', rect.width + 'x' + rect.height);
      konvaEnsurePreviewStage(container, rect.width, rect.height);
      if (!konvaPreviewLayer) { console.warn('[Konva] no layer after ensureStage'); return; }
      konvaPreviewLayer.destroyChildren();
      await konvaBuildSceneNodes(konvaPreviewLayer, scene, KONVA_DEV_MODE);
      konvaPreviewLayer.batchDraw();
      konvaPreviewLastKey = sceneKey || '';
      console.log('[Konva] preview rendered, nodes on layer:', konvaPreviewLayer.getChildren().length, 'stage container:', konvaPreviewStage?.container()?.id);
    }

    function konvaResizePreview(container) {
      if (!container || !konvaPreviewStage) return;
      const rect = container.getBoundingClientRect();
      if (!rect.width || !rect.height) return;
      konvaPreviewStage.width(rect.width);
      konvaPreviewStage.height(rect.height);
      konvaPreviewLayer.scaleX(rect.width / KONVA_EXPORT_W);
      konvaPreviewLayer.scaleY(rect.height / KONVA_EXPORT_H);
      konvaPreviewLayer.batchDraw();
    }
    ...
    function konvaSvgPolaroidFrame(o = {}) {
      const { x = 28, y = 14, width: w = 44, height: h = 60 } = o;
      const px = x + 3, py = y + 3, pw = w - 6, ph = h - 18;
      return `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="4" fill="#ffffff" stroke="#d8dde5" stroke-width="1.4"/><rect x="${px}" y="${py}" width="${pw}" height="${ph}" rx="2.5" fill="#eef2f7" stroke="#e4e9f0" stroke-width="0.8"/>${konvaSvgText(['ТУТ БУДЕТ', 'ФОТО'], { x: px + pw / 2, y: py + ph / 2 - 5, lineHeight: 7, fontSize: 5.4, fill: '#97a1af', letterSpacing: 0.28 })}`;
    }
    function konvaBuildTemplatePreviewSvg(kind, mode) {
      ...
      switch (kind) {
        ...
        case 'polaroid-top-text': c += konvaSvgText(['ВАШ ТЕКСТ'], { x: 50, y: 13, lineHeight: 8, fontSize: 6.2 }); c += konvaSvgPolaroidFrame({ x: 29, y: 21, width: 42, height: 58 }); break;
        case 'polaroid-bottom-text': c += konvaSvgPolaroidFrame({ x: 29, y: 11, width: 42, height: 58 }); c += konvaSvgText(['ВАШ ТЕКСТ'], { x: 50, y: 83, lineHeight: 8, fontSize: 6.2 }); break;
        case 'polaroid-diagonal-text': c += konvaSvgPolaroidFrame({ x: 29, y: 11, width: 42, height: 58 }); c += konvaSvgText(['ВАШ ТЕКСТ'], { x: 67, y: 73, lineHeight: 8, fontSize: 6, rotation: -18 }); break;
        case 'polaroid-inside-text': c += konvaSvgPolaroidFrame({ x: 24, y: 12, width: 52, height: 66 }); c += konvaSvgText(tl, { x: 68, y: 72, lineHeight: 7, fontSize: 5.8, anchor: 'end' }); break;
    ...
    function buildKonvaSceneParams() {
      ...
      return {
        ...
        exportLineHeight: textRules.exportLineHeight || DEFAULT_PREVIEW_TEXT_RULES.exportLineHeight,
        exportFontScale: efs,
        ...
        frameSrc: isPolaroidPhotoTemplate(selectedTemplate) ? normalizeExportImageUrl(POLAROID_FRAME_IMAGE_URL) : null,
    ...
    function normalizeTemplateId(templateId = '') {
      const legacyTemplateMap = {
        polaroid: 'polaroid-top-text',
        'polaroid-tilt-text': 'polaroid-diagonal-text',
      };
    ...
    function isSimplePhotoTemplate(template = getSelectedTemplate()) {
      ...
      return template.kind === 'simple-photo';
    }
    function isPhotoTopTextTemplate(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return template.kind === 'photo-top-text';
    }
    function isPhotoBottomTextTemplate(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return template.kind === 'photo-bottom-text';
    }
    function isPhotoDiagonalTextTemplate(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return template.kind === 'photo-diagonal-text';
    }
    function isPolaroidTopTextTemplate(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return template.kind === 'polaroid-top-text';
    }
    function isPolaroidBottomTextTemplate(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return template.kind === 'polaroid-bottom-text';
    }
    function isPolaroidDiagonalTextTemplate(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return template.kind === 'polaroid-diagonal-text';
    }
    function isPolaroidInsideTextTemplate(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return template.kind === 'polaroid-inside-text';
    }
    function isPolaroidPhotoTemplate(template = getSelectedTemplate()) {
      return isPolaroidTopTextTemplate(template)
        || isPolaroidBottomTextTemplate(template)
        || isPolaroidDiagonalTextTemplate(template)
        || isPolaroidInsideTextTemplate(template);
    }
    ...
    function isPolaroidTextTemplate(template = getSelectedTemplate()) {
      return isPolaroidTopTextTemplate(template)
        || isPolaroidBottomTextTemplate(template)
        || isPolaroidDiagonalTextTemplate(template)
        || isPolaroidInsideTextTemplate(template);
    ...
    function getTemplatePhotoLayout(
      ...
      if (isPhotoTopTextTemplate(template)) {
        const textMetrics = getPreviewTextRenderMetrics(template, printSize, resolvedLineCount, {
          hasStandardUpload: false,
        });
        const previewTextStyle = getPreviewTextBlockStyleConfig(template, printSize, resolvedLineCount);
        const previewTopPercent = Number.parseFloat(String(previewTextStyle.previewTop || '').replace('%', ''));
        const previewFontSize = Number.parseFloat(String(previewTextStyle.fontSize || '').replace('px', '')) || 0;
        const previewLineHeight = Number.parseFloat(String(previewTextStyle.lineHeight || '')) || 1.14;
        const previewScale = getCurrentPrintScale(printSize?.id, template);
        const previewTextHeight = previewFontSize * previewLineHeight * resolvedLineCount * previewScale;
        const exportLineHeight = Number(textMetrics.lineHeight) || 0;
        const exportFontSize = Number(textMetrics.fontSize) || 0;
        const exportTextBottom = Number(textMetrics.y)
          + (Math.max(0, resolvedLineCount - 1) * exportLineHeight)
          + (exportFontSize * PHOTO_TOP_TEXT_EXPORT_DESCENT_RATIO);
        const previewPhotoTopPercent = Number.isFinite(previewTopPercent)
          ? previewTopPercent + (((previewTextHeight + PHOTO_TOP_TEXT_PREVIEW_GAP) / GARMENT_ARTBOARD_HEIGHT) * 100)
          : ((exportTextBottom + PHOTO_TOP_TEXT_EXPORT_GAP) / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100;
        const exportY = Math.round(exportTextBottom + PHOTO_TOP_TEXT_EXPORT_GAP);

        return {
          exportX: (CHECKOUT_EXPORT_ARTBOARD_WIDTH - exportWidth) / 2,
          exportY,
          exportWidth,
          exportHeight,
          previewLeft: '50%',
          previewTop: `${Math.round(previewPhotoTopPercent * 1000) / 1000}%`,
          previewWidth: `${(exportWidth / CHECKOUT_EXPORT_ARTBOARD_WIDTH) * 100}%`,
          previewHeight: `${(exportHeight / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100}%`,
        };
      }
      ...
      return {
        exportX: (CHECKOUT_EXPORT_ARTBOARD_WIDTH - exportWidth) / 2,
        exportY: resolvedExportTop,
        exportWidth,
        exportHeight,
        previewLeft: '50%',
        previewTop: `${(resolvedExportTop / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100}%`,
        previewWidth: `${(exportWidth / CHECKOUT_EXPORT_ARTBOARD_WIDTH) * 100}%`,
        previewHeight: `${(exportHeight / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100}%`,
      };
    }
    function getPolaroidFrameLayout(
      template = getSelectedTemplate(),
      photoPrintSize = getSelectedPhotoPrintSize(undefined, template)
    ) {
      const photoConfig = getTemplatePhotoConfig(template);
      const photoExportWidth = Number(photoPrintSize?.exportWidth) || 110;
      const photoExportHeight = Number(photoPrintSize?.exportHeight) || 138;
      const frameExportWidth = photoExportWidth / POLAROID_FRAME_INNER_WIDTH_RATIO;
      const frameExportHeight = photoExportHeight / POLAROID_FRAME_INNER_HEIGHT_RATIO;
      const exportX = (CHECKOUT_EXPORT_ARTBOARD_WIDTH - frameExportWidth) / 2;
      const exportY = Number(photoConfig?.exportTop) || POLAROID_FRAME_EXPORT_TOP;
      const photoExportX = exportX + (frameExportWidth * POLAROID_FRAME_INNER_X_RATIO);
      const photoExportY = exportY + (frameExportHeight * POLAROID_FRAME_INNER_Y_RATIO);
      return {
        exportX,
        exportY,
        exportWidth: frameExportWidth,
        exportHeight: frameExportHeight,
        photoExportX,
        photoExportY,
        photoExportWidth,
        photoExportHeight,
        previewLeft: '50%',
        previewTop: `${(exportY / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100}%`,
        previewWidth: `${(frameExportWidth / CHECKOUT_EXPORT_ARTBOARD_WIDTH) * 100}%`,
        previewHeight: `${(frameExportHeight / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100}%`,
        photoPreviewLeft: `${POLAROID_FRAME_INNER_X_RATIO * 100}%`,
        photoPreviewTop: `${POLAROID_FRAME_INNER_Y_RATIO * 100}%`,
        photoPreviewWidth: `${POLAROID_FRAME_INNER_WIDTH_RATIO * 100}%`,
        photoPreviewHeight: `${POLAROID_FRAME_INNER_HEIGHT_RATIO * 100}%`,
      };
    }
    function getPolaroidDiagonalTextLayout(
      template = getSelectedTemplate(),
      printSize = getSelectedPrintSize(undefined, template),
      lineCount = 1
    ) {
      if (!isPolaroidDiagonalTextTemplate(template)) return null;
      const frameLayout = getPolaroidFrameLayout(template, getSelectedPhotoPrintSize(undefined, template));
      const exportX = Number(frameLayout.exportX || 0) + (Number(frameLayout.exportWidth || 0) * POLAROID_DIAGONAL_TEXT_X_RATIO);
      const previewTopSource = Number(frameLayout.exportY || 0) + (Number(frameLayout.exportHeight || 0) * POLAROID_DIAGONAL_TEXT_PREVIEW_TOP_RATIO);
      const exportY = Number(frameLayout.exportY || 0) + (Number(frameLayout.exportHeight || 0) * POLAROID_DIAGONAL_TEXT_EXPORT_BASELINE_RATIO);
      return {
        rotation: POLAROID_DIAGONAL_TEXT_ROTATION,
        exportX,
        exportY,
    ...
    function getPolaroidInsideTextLayout(
      template = getSelectedTemplate(),
      printSize = getSelectedPrintSize(undefined, template),
      lineCount = 1
    ) {
      if (!isPolaroidInsideTextTemplate(template)) return null;
      const frameLayout = getPolaroidFrameLayout(template, getSelectedPhotoPrintSize(undefined, template));
      const exportX = Number(frameLayout.exportX || 0) + (Number(frameLayout.exportWidth || 0) * POLAROID_INSIDE_TEXT_X_RATIO);
      const exportY = Number(frameLayout.exportY || 0) + (Number(frameLayout.exportHeight || 0) * POLAROID_INSIDE_TEXT_Y_RATIO);
      const exportWidth = Number(frameLayout.exportWidth || 0) * POLAROID_INSIDE_TEXT_WIDTH_RATIO;
      const exportHeight = Number(frameLayout.exportHeight || 0) * POLAROID_INSIDE_TEXT_HEIGHT_RATIO;
      const exportPaddingX = exportWidth * POLAROID_INSIDE_TEXT_PADDING_X_RATIO;
      const exportPaddingY = exportHeight * POLAROID_INSIDE_TEXT_PADDING_Y_RATIO;
      return {
        exportX,
        exportY,
        exportWidth,
        ...
        exportPaddingX,
        exportPaddingY,
        exportInnerY: exportY + exportPaddingY,
        exportInnerHeight: Math.max(1, exportHeight - (exportPaddingY * 2)),
        exportRightX: exportX + exportWidth - exportPaddingX,
        exportCenterX: exportX + (exportWidth / 2),
        previewLeft: `${((exportX + (exportWidth / 2)) / CHECKOUT_EXPORT_ARTBOARD_WIDTH) * 100}%`,
        previewTop: `${(exportY / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100}%`,
        previewWidth: `${(exportWidth / CHECKOUT_EXPORT_ARTBOARD_WIDTH) * 100}%`,
        previewHeight: `${(exportHeight / CHECKOUT_EXPORT_ARTBOARD_HEIGHT) * 100}%`,
        localLeft: `${POLAROID_INSIDE_TEXT_X_RATIO * 100}%`,
        localTop: `${POLAROID_INSIDE_TEXT_Y_RATIO * 100}%`,
        localWidth: `${POLAROID_INSIDE_TEXT_WIDTH_RATIO * 100}%`,
        localHeight: `${POLAROID_INSIDE_TEXT_HEIGHT_RATIO * 100}%`,
        localPaddingX: `${POLAROID_INSIDE_TEXT_PADDING_X_RATIO * 100}%`,
        localPaddingY: `${POLAROID_INSIDE_TEXT_PADDING_Y_RATIO * 100}%`,
      };
    }
    ...
    function getGarmentPreviewFocusConfig(template = getSelectedTemplate()) {
      ...
      if (isPhotoTextTemplate(template) || isPolaroidTextTemplate(template)) {
        return {
          scale: isCompactViewport ? 1.33 : 1.45,
          focusX: 0.5,
          focusY: (isPhotoBottomTextTemplate(template)
            || isPhotoDiagonalTextTemplate(template)
            || isPolaroidBottomTextTemplate(template)
            || isPolaroidDiagonalTextTemplate(template)) ? 0.46 : 0.45,
        };
      }
    ...
    function getPreviewTextBlockStyleConfig(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1) {
      ...
      const previewTextFocusScale = isPolaroidTextTemplate(template) ? 1 : previewFocusScale;
      ...
      if (isPolaroidInsideTextTemplate(template)) {
        const insideLayout = getPolaroidInsideTextLayout(template, printSize, lineCount);
        if (insideLayout) {
          previewTop = insideLayout.previewTop;
          previewLeft = insideLayout.previewLeft;
          previewRight = 'auto';
          previewTextAlign = 'center';
          previewTransformOrigin = 'center top';
          previewWidth = insideLayout.previewWidth;
        }
      } else if (isPhotoDiagonalTextTemplate(template) || isPolaroidDiagonalTextTemplate(template)) {
        const diagonalLayout = isPolaroidDiagonalTextTemplate(template)
          ? getPolaroidDiagonalTextLayout(template, printSize, lineCount)
          : getPhotoDiagonalTextLayout(template, printSize, lineCount);
        if (diagonalLayout) {
      ...
      } else if (isPhotoBottomTextTemplate(template) || isPolaroidBottomTextTemplate(template)) {
        const layout = isPolaroidBottomTextTemplate(template)
          ? getPolaroidFrameLayout(template, getSelectedPhotoPrintSize(undefined, template))
          : getTemplatePhotoLayout(template, getSelectedPhotoPrintSize(undefined, template), {
            printSize,
            lineCount,
          });
        ...
        const previewGap = isPolaroidBottomTextTemplate(template)
          ? POLAROID_BOTTOM_TEXT_PREVIEW_GAP
          : PHOTO_BOTTOM_TEXT_PREVIEW_GAP;
        if (Number.isFinite(previewTopPercent) && Number.isFinite(previewHeightPercent)) {
          previewTop = `${previewTopPercent + previewHeightPercent + ((previewGap / GARMENT_ARTBOARD_HEIGHT) * 100)}%`;
        }
      } else if (isPolaroidTopTextTemplate(template)) {
        const frameLayout = getPolaroidFrameLayout(template, getSelectedPhotoPrintSize(undefined, template));
        const frameTopPercent = Number.parseFloat(String(frameLayout.previewTop || '').replace('%', ''));
        const previewTextHeight = basePreviewFontSize * previewFontScale * previewTextFocusScale
          * (Number(rules.lineHeight) || DEFAULT_PREVIEW_TEXT_RULES.lineHeight)
          * Math.max(1, Number(lineCount) || 1)
          * currentScale;
        if (Number.isFinite(frameTopPercent)) {
          previewTop = `${frameTopPercent - (((previewTextHeight + POLAROID_TOP_TEXT_PREVIEW_GAP) / GARMENT_ARTBOARD_HEIGHT) * 100)}%`;
        }
      }
      ...
      } else if (isPolaroidDiagonalTextTemplate(template)) {
        transformParts.push(`rotate(${POLAROID_DIAGONAL_TEXT_ROTATION}deg)`);
      }
    ...
    function getPreviewTextRenderMetrics(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1, options = {}) {
      ...
      if (config) {
        ...
        const lineHeight = (rules.exportLineHeight || DEFAULT_PREVIEW_TEXT_RULES.exportLineHeight) * exportFontScale;
        if (isPolaroidInsideTextTemplate(template)) {
          const insideLayout = getPolaroidInsideTextLayout(template, printSize, lineCount);
          if (insideLayout) {
            const contentHeight = fontSize + (Math.max(0, lineCount - 1) * lineHeight);
            const fitScale = Math.min(1, insideLayout.exportInnerHeight / Math.max(1, contentHeight));
            const fittedFontSize = fontSize * fitScale;
            const fittedLineHeight = lineHeight * fitScale;
            const fittedContentHeight = fittedFontSize + (Math.max(0, lineCount - 1) * fittedLineHeight);
            const topInset = Math.max(0, (insideLayout.exportInnerHeight - fittedContentHeight) / 2);
            return {
              x: insideLayout.exportRightX,
              anchor: 'end',
              fontSize: fittedFontSize,
              lineHeight: fittedLineHeight,
              y: insideLayout.exportInnerY + topInset + (fittedFontSize * POLAROID_INSIDE_TEXT_BASELINE_RATIO),
            };
          }
        }
        if (isPhotoDiagonalTextTemplate(template) || isPolaroidDiagonalTextTemplate(template)) {
          const diagonalLayout = isPolaroidDiagonalTextTemplate(template)
            ? getPolaroidDiagonalTextLayout(template, printSize, lineCount)
            : getPhotoDiagonalTextLayout(template, printSize, lineCount);
          if (diagonalLayout) {
        ...
        if (isPhotoBottomTextTemplate(template) || isPolaroidBottomTextTemplate(template)) {
          const layout = isPolaroidBottomTextTemplate(template)
            ? getPolaroidFrameLayout(template, getSelectedPhotoPrintSize(undefined, template))
            : getTemplatePhotoLayout(template, getSelectedPhotoPrintSize(undefined, template), {
              printSize,
          ...
          const textTop = Number(layout?.exportY || 0)
            ...
            + (isPolaroidBottomTextTemplate(template) ? POLAROID_BOTTOM_TEXT_EXPORT_GAP : PHOTO_BOTTOM_TEXT_EXPORT_GAP);
          return {
            x: rules.exportX || 500,
        ...
        if (isPolaroidTopTextTemplate(template)) {
          const frameLayout = getPolaroidFrameLayout(template, getSelectedPhotoPrintSize(undefined, template));
          return {
            x: rules.exportX || 500,
            anchor: rules.exportAnchor || 'middle',
            fontSize,
            lineHeight,
            y: Number(frameLayout.exportY || 0)
              - POLAROID_TOP_TEXT_EXPORT_GAP
              - (fontSize * PHOTO_TOP_TEXT_EXPORT_DESCENT_RATIO)
              - (Math.max(0, Number(lineCount) - 1) * lineHeight),
          };
        }
    ...
    function renderTemplatePreviewPolaroidFrame(options = {}) {
      const {
        x = 28,
        y = 14,
        width = 44,
      ...
      const photoX = x + 3;
      const photoY = y + 3;
      const photoWidth = width - 6;
      const photoHeight = height - 18;
      return `
        <rect x="${x}" y="${y}" width="${width}" height="${height}" rx="4" fill="#ffffff" stroke="#d8dde5" stroke-width="1.4"/>
        <rect x="${photoX}" y="${photoY}" width="${photoWidth}" height="${photoHeight}" rx="2.5" fill="${photoFill}" stroke="#e4e9f0" stroke-width="0.8"/>
        ${showPhotoPlaceholder ? renderTemplatePreviewText(['ТУТ БУДЕТ', 'ФОТО'], {
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function createCheckoutCompositePreviewUrl(options = {}) {
      ...
      const isPolaroidTemplate = isPolaroidPhotoTemplate(selectedTemplate);
      ...
      const polaroidFrameLayout = isPolaroidTemplate
        ? getPolaroidFrameLayout(selectedTemplate, selectedPhotoPrintSize)
        : null;
      const polaroidFrameHref = isPolaroidTemplate
        ? await fetchImageAsDataUrl(POLAROID_FRAME_IMAGE_URL)
        : '';
      const uploadPreviewUrl = isPolaroidTemplate && state.uploads.length
        ? await createTemplatePhotoDataUrl(
          polaroidFrameLayout?.photoExportWidth || 0,
          polaroidFrameLayout?.photoExportHeight || 0
        )
        : '';
      ...
      if (state.selectedCustom) {
        ...
        const polaroidInsideTextLayout = isPolaroidInsideTextTemplate(selectedTemplate)
          ? getPolaroidInsideTextLayout(selectedTemplate, selectedPrintSize, textLines.length)
          : null;
        const textMetrics = templateSupportsText(selectedTemplate)
          ? getPreviewTextRenderMetrics(selectedTemplate, selectedPrintSize, textLines.length, {
            hasStandardUpload: false,
          })
          : null;
        const rawTextMarkup = textMetrics ? renderSvgTextBlock(textLines, {
          x: textMetrics.x,
          y: textMetrics.y,
        ...
        const textMarkup = polaroidInsideTextLayout && rawTextMarkup ? `
          <clipPath id="checkoutPolaroidInsideTextClip">
            <rect
              x="${polaroidInsideTextLayout.exportX}"
              y="${polaroidInsideTextLayout.exportY}"
              width="${polaroidInsideTextLayout.exportWidth}"
              height="${polaroidInsideTextLayout.exportHeight}"
            />
          </clipPath>
          <g clip-path="url(#checkoutPolaroidInsideTextClip)">
            ${rawTextMarkup}
          </g>
        ` : rawTextMarkup;
        const polaroidMarkup = `
          ${uploadPreviewUrl ? `<image href="${escapeSvg(uploadPreviewUrl)}" x="${polaroidFrameLayout.photoExportX}" y="${polaroidFrameLayout.photoExportY}" width="${polaroidFrameLayout.photoExportWidth}" height="${polaroidFrameLayout.photoExportHeight}" preserveAspectRatio="xMidYMid slice"/>` : ''}
          ${polaroidFrameHref ? `<image href="${escapeSvg(polaroidFrameHref)}" x="${polaroidFrameLayout.exportX}" y="${polaroidFrameLayout.exportY}" width="${polaroidFrameLayout.exportWidth}" height="${polaroidFrameLayout.exportHeight}" preserveAspectRatio="xMidYMid meet"/>` : ''}
        `;
        designMarkup = isPolaroidTopTextTemplate(selectedTemplate)
          ? `${textMarkup}${polaroidMarkup}`
          : `${polaroidMarkup}${textMarkup}`;
      } else if (selectedTemplate) {
        if (isPhotoTextTemplate(selectedTemplate) && templatePhotoLayout) {
          const textMetrics = getPreviewTextRenderMetrics(selectedTemplate, selectedPrintSize, textLines.length, {
            hasStandardUpload: false,
          });
          const textMarkup = renderSvgTextBlock(textLines, {
            x: textMetrics.x,
            y: textMetrics.y,
            lineHeight: textMetrics.lineHeight,
            fontSize: textMetrics.fontSize,
          ...
          const photoMarkup = standardUploadPreviewUrl ? `
            ...
            />
          ` : '';
          designMarkup = (isPhotoBottomTextTemplate(selectedTemplate) || isPhotoDiagonalTextTemplate(selectedTemplate))
            ? `${photoMarkup}${textMarkup}`
            : `${textMarkup}${photoMarkup}`;
        } else if (templatePhotoLayout) {
          designMarkup = standardUploadPreviewUrl ? `
            <image
              href="${escapeSvg(standardUploadPreviewUrl)}"
              x="${templatePhotoLayout.exportX}"
              y="${templatePhotoLayout.exportY}"
    ...
    async function renderCheckoutCompositePreviewCanvas(canvas) {
      ...
      if (isPolaroidTemplate && canUseDomPreview) {
      ...
      if (isPolaroidTemplate) {
        const polaroidFrameLayout = getPolaroidFrameLayout(selectedTemplate, selectedPhotoPrintSize);
        const polaroidInsideTextLayout = isPolaroidInsideTextTemplate(selectedTemplate)
          ? getPolaroidInsideTextLayout(selectedTemplate, selectedPrintSize, textLines.length)
          : null;
        ...
        if (hasPolaroidUpload) {
          await drawCurrentUploadToCanvas(
            ctx,
            polaroidFrameLayout.photoExportX,
            polaroidFrameLayout.photoExportY,
            polaroidFrameLayout.photoExportWidth,
            polaroidFrameLayout.photoExportHeight
          );
        } else {
        ...
        await drawCanvasContainImage(
          ctx,
          polaroidFrameSource,
          polaroidFrameLayout.exportX,
          polaroidFrameLayout.exportY,
          polaroidFrameLayout.exportWidth,
          polaroidFrameLayout.exportHeight
        );
        if (templateSupportsText(selectedTemplate)) {
          const textMetrics = getPreviewTextRenderMetrics(selectedTemplate, selectedPrintSize, textLines.length, {
            hasStandardUpload: false,
          });
          if (polaroidInsideTextLayout) {
            ctx.save();
            buildRoundedRectPath(
              ctx,
              polaroidInsideTextLayout.exportX,
              polaroidInsideTextLayout.exportY,
              polaroidInsideTextLayout.exportWidth,
              polaroidInsideTextLayout.exportHeight,
              0
            );
            ctx.clip();
          }
    ...
    async function createCheckoutCompositePreviewUiSourceUrl() {
      ...
      const preferCanvasPreview = templateSupportsText(selectedTemplate)
        || isPolaroidPhotoTemplate(selectedTemplate)
    ...
    function shouldUseCheckoutCompositePreviewInUi(template = getSelectedTemplate()) {
      ...
      return isPolaroidPhotoTemplate(template);
    }
      ...
      if (state.step === 4) {
        ...
        if (textInput) {
          ...
          textInput.oninput = (e) => {
            ...
            const normalizedValue = normalizeTextInputValue(e.target.value, template, printSize);
...
function renderLeftSummary() {
    ...
    function drawCanvasPhotoPlaceholder(ctx, x, y, width, height, options = {}) {
      ...
      const lines = polaroid ? ['ТУТ БУДЕТ', 'ФОТО'] : ['ТУТ БУДЕТ', 'ВАШЕ ФОТО'];
      ctx.save();
      buildRoundedRectPath(ctx, x, y, width, height, radius);
      ctx.fillStyle = '#ffffff';
      ctx.fill();
      ctx.lineWidth = polaroid ? 0.8 : 1.2;
      ctx.strokeStyle = polaroid ? '#e4e9f0' : '#d5dae1';
      if (!polaroid) ctx.setLineDash([10, 8]);
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.fillStyle = polaroid ? '#97a1af' : '#7b8593';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'alphabetic';
      const fontSize = Math.max(6, Math.min(width * 0.09, polaroid ? 9 : 10));
    ...
    async function renderPolaroidPreviewCanvasFromDom(ctx, domMetrics) {
      ...
      const textBlocks = Array.from(designLayer.querySelectorAll('.emb-block, .polaroid-text')).filter((node) => node instanceof HTMLElement);
      ...
      await withCanvasElementTransform(ctx, photo, domMetrics, async ({ width, height }) => {
        if (hasUpload) {
      ...
      const frameSource = frame.currentSrc || frame.getAttribute('src') || '';
      await withCanvasElementTransform(ctx, frame, domMetrics, async ({ width, height }) => {
        await drawCanvasContainImage(ctx, frameSource, 0, 0, width, height);
      });
      for (const element of textBlocks) {
        await withCanvasElementTransform(ctx, element, domMetrics, async ({ width, height }) => {
          drawCanvasTextElementInBox(ctx, element, {
            x: 0,
            y: 0,
            width,
            height,
      ...
      if (isPolaroidInsideTextTemplate(selectedTemplate) && textLines.length) {
        const insideLayout = getPolaroidInsideTextLayout(selectedTemplate, selectedPrintSize, textLines.length);
        const textMetrics = getPreviewTextRenderMetrics(selectedTemplate, selectedPrintSize, textLines.length, {
          hasStandardUpload: false,
        });
        if (insideLayout && textMetrics) {
          ctx.save();
          buildRoundedRectPath(
            ctx,
            insideLayout.exportX,
            insideLayout.exportY,
            insideLayout.exportWidth,
            insideLayout.exportHeight,
            0
          );
          ctx.clip();
          drawCanvasTextBlock(ctx, textLines, {
    ...
    function getDesignLayerRenderState() {
      ...
      const selectedPrintSize = getSelectedPrintSize(undefined, selectedTemplate);
      const currentScale = getCurrentPrintScale(selectedPrintSize?.id, selectedTemplate);
      const isPolaroid = state.step !== 1 && isPolaroidPhotoTemplate(selectedTemplate);
      ...
      const polaroidPhotoPlaceholderHtml = `<div class="photo-placeholder photo-placeholder--polaroid">ТУТ БУДЕТ<br>ВАШЕ ФОТО</div>`;
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        const polaroidFrameLayout = getPolaroidFrameLayout(selectedTemplate, selectedPhotoPrintSize);
        const polaroidInsideTextLayout = isPolaroidInsideTextTemplate(selectedTemplate)
          ? getPolaroidInsideTextLayout(selectedTemplate, selectedPrintSize, previewTextLines.length)
          : null;
        const polaroidInsideTextMetrics = polaroidInsideTextLayout
          ? getPreviewTextRenderMetrics(selectedTemplate, selectedPrintSize, previewTextLines.length, {
            hasStandardUpload: false,
          })
          : null;
        const polaroidFrameSrc = escapeAttr(normalizeProductImageUrl(POLAROID_FRAME_IMAGE_URL));
        const insideTextSvgMarkup = (polaroidInsideTextLayout && polaroidInsideTextMetrics)
          ? renderPreviewSvgScene({
            width: polaroidInsideTextLayout.exportWidth,
            height: polaroidInsideTextLayout.exportHeight,
            content: renderSvgTextBlock(previewTextLines, {
              x: polaroidInsideTextMetrics.x - polaroidInsideTextLayout.exportX,
              y: polaroidInsideTextMetrics.y - polaroidInsideTextLayout.exportY,
              lineHeight: polaroidInsideTextMetrics.lineHeight,
              fontSize: polaroidInsideTextMetrics.fontSize,
              fill: state.textColor,
              fontFamily: getFontFamily(state.font),
              anchor: polaroidInsideTextMetrics.anchor,
              fontWeight: previewTextRules.fontWeight,
              letterSpacing: getPreviewTextLetterSpacingValue(selectedTemplate, selectedPrintSize, polaroidInsideTextMetrics.fontSize),
              rotation: polaroidInsideTextMetrics.rotation,
            }),
          })
          : '';
        const insideTextBlockHtml = polaroidInsideTextLayout ? `
          <div
            class="polaroid-text-inside-preview"
            style="position:absolute; left:${polaroidInsideTextLayout.localLeft}; top:${polaroidInsideTextLayout.localTop}; width:${polaroidInsideTextLayout.localWidth}; height:${polaroidInsideTextLayout.localHeight}; overflow:hidden; pointer-events:none;"
          >
            ${insideTextSvgMarkup}
          </div>
        ` : '';
        const textBlockHtml = templateSupportsText(selectedTemplate) && !polaroidInsideTextLayout ? `
        ...
        ` : '';
        const polaroidBlockHtml = `
          <div style="position:absolute; left:${polaroidFrameLayout.previewLeft}; top:${polaroidFrameLayout.previewTop}; width:${polaroidFrameLayout.previewWidth}; height:${polaroidFrameLayout.previewHeight}; transform:translateX(-50%);">
            ${activeUpload ? `
              <div
                style="position:absolute; left:${polaroidFrameLayout.photoPreviewLeft}; top:${polaroidFrameLayout.photoPreviewTop}; width:${polaroidFrameLayout.photoPreviewWidth}; height:${polaroidFrameLayout.photoPreviewHeight}; overflow:hidden; border:none; border-radius:0; background:transparent;"
              >
                ${uploadImageHtml || polaroidPhotoPlaceholderHtml}
              </div>
            ` : `
              <div
                style="position:absolute; left:${polaroidFrameLayout.photoPreviewLeft}; top:${polaroidFrameLayout.photoPreviewTop}; width:${polaroidFrameLayout.photoPreviewWidth}; height:${polaroidFrameLayout.photoPreviewHeight}; overflow:hidden; border:none; border-radius:0; background:transparent;"
              >
                ${polaroidPhotoPlaceholderHtml}
              </div>
            `}
            <img src="${polaroidFrameSrc}" alt="" style="position:absolute; inset:0; width:100%; height:100%; object-fit:contain; pointer-events:none; user-select:none;">
            ${insideTextBlockHtml}
          </div>
        `;
        html = isPolaroidTopTextTemplate(selectedTemplate)
          ? `${textBlockHtml}${polaroidBlockHtml}`
          : `${polaroidBlockHtml}${textBlockHtml}`;
        ...
        `;
        html = (isPhotoBottomTextTemplate(selectedTemplate) || isPhotoDiagonalTextTemplate(selectedTemplate))
    ...
    function renderVisualizer() {
      ...
      const designLayer = document.getElementById('designLayer');
      console.log('[Konva] renderVisualizer: available=' + isKonvaAvailable(), 'designLayer=' + !!designLayer, 'step=' + state.step);
      if (isKonvaAvailable() && designLayer) {
        // Force pointer-events and remove transform interference for Konva dev mode drag
        if (KONVA_DEV_MODE) {
          designLayer.style.pointerEvents = 'auto';
          designLayer.style.zIndex = '20';
          const garmentEl = document.getElementById('garment');
          if (garmentEl) {
            garmentEl.style.transform = 'none';
            garmentEl.style.willChange = 'auto';
          }
        }
        const sceneParams = buildKonvaSceneParams();
        console.log('[Konva] scene params: kind=' + sceneParams.templateKind, 'hide=' + sceneParams.hideDesign, 'custom=' + sceneParams.isCustom, 'textLines=' + sceneParams.textLines.length);
        ...
        if (designLayer.querySelector('.emb-block, .image-box, .polaroid-wrap')) {
          designLayer.innerHTML = '';
        }
        konvaRenderPreview(designLayer, scene, sceneKey).catch((err) => {
    ...
    window.addEventListener('resize', () => {
      if (isKonvaAvailable()) {
        const designLayer = document.getElementById('designLayer');
        if (designLayer) konvaResizePreview(designLayer);
      }
    });
    window.addEventListener('beforeunload', (event) => {
      ...
      const snapshot = getPersistenceSnapshot();
      persistenceWriteChain = persistenceWriteChain.then(() => writePersistedSnapshot(snapshot));
      if (!orderSubmitInFlight && hasMeaningfulPersistedSnapshot(snapshot)) {
        event.preventDefault();
        event.returnValue = '';
      }
    });
