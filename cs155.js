...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function setupPreviewZoom() {
      ...
      if (!garmentWrap || !garmentEl) return;
      ...
      const applyTransform = (originX, originY, zoom, panX, panY, animate) => {
        garmentEl.style.transition = animate ? 'transform .25s ease' : 'none';
        garmentEl.style.transformOrigin = `${originX}% ${originY}%`;
        garmentEl.style.transform = zoom > 1
      ...
      if (isDesktop) {
        garmentWrap.addEventListener('mouseenter', (e) => {
          ...
          applyTransform(ox, oy, DESKTOP_ZOOM, 0, 0, true);
        }, { signal });
        ...
        garmentWrap.addEventListener('mousemove', (e) => {
          ...
          garmentEl.style.transformOrigin = `${ox}% ${oy}%`;
        }, { signal });
