...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function konvaUpdateDebugPanel(node) {
      ...
      const sx = Math.round((node.scaleX ? node.scaleX() : 1) * 1000) / 1000;
      const sy = Math.round((node.scaleY ? node.scaleY() : 1) * 1000) / 1000;
      const w = Math.round((node.width ? node.width() * sx : 0) * 10) / 10;
      const h = Math.round((node.height ? node.height() * sy : 0) * 10) / 10;
