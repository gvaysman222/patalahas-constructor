...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function buildTemplatePreviewSvgMarkup(template, mode = 'template') {
      if (!template) return '';
      if (mode === 'custom') {
        return renderPreviewSvgScene({
          content: `
            <rect width="100" height="100" rx="18" fill="#f4f5f7"/>
            <rect x="16" y="16" width="68" height="68" rx="18" fill="#ffffff" stroke="#d8dde5" stroke-width="1.4"/>
            ${renderTemplatePreviewText(['ИНД', 'ДИЗАЙН'], {
              x: 50,
              y: 47,
              lineHeight: 11,
              fontSize: 9,
      ...
      const textLines = ['ВАШ', 'ТЕКСТ'];
      let content = '<rect width="100" height="100" rx="18" fill="#f4f5f7"/>';
      switch (template.kind) {
        case 'emb-center':
          content += renderTemplatePreviewText(textLines, {
            x: 50,
            y: 45,
            lineHeight: 11,
            fontSize: 9,
          ...
          break;
        case 'emb-left':
          content += renderTemplatePreviewText(['ВАШ ТЕКСТ'], {
            x: 20,
            y: 46,
            lineHeight: 10,
            fontSize: 7.6,
          ...
          break;
        case 'simple-photo':
          content += renderTemplatePreviewPhotoBox();
          break;
        case 'photo-top-text':
          content += renderTemplatePreviewText(['ВАШ ТЕКСТ'], {
            x: 50,
          ...
          content += renderTemplatePreviewPhotoBox({ x: 18, y: 24, width: 64, height: 56 });
          break;
        case 'photo-bottom-text':
          content += renderTemplatePreviewPhotoBox({ x: 18, y: 14, width: 64, height: 50 });
          content += renderTemplatePreviewText(['ВАШ ТЕКСТ'], {
            x: 50,
            y: 78,
            lineHeight: 8,
          ...
          break;
        case 'photo-diagonal-text':
          content += renderTemplatePreviewPhotoBox({ x: 18, y: 18, width: 64, height: 56 });
          content += renderTemplatePreviewText(['ВАШ ТЕКСТ'], {
            x: 69,
            y: 73,
            lineHeight: 8,
        ...
        case 'polaroid-top-text':
          ...
          content += renderTemplatePreviewPolaroidFrame({ x: 29, y: 21, width: 42, height: 58 });
          break;
        case 'polaroid-bottom-text':
          content += renderTemplatePreviewPolaroidFrame({ x: 29, y: 11, width: 42, height: 58 });
          content += renderTemplatePreviewText(['ВАШ ТЕКСТ'], {
            x: 50,
            y: 83,
            lineHeight: 8,
        ...
        case 'polaroid-diagonal-text':
          ...
          break;
        case 'polaroid-inside-text':
          content += renderTemplatePreviewPolaroidFrame({ x: 24, y: 12, width: 52, height: 66 });
          content += renderTemplatePreviewText(textLines, {
            x: 68,
            y: 72,
            lineHeight: 7,
      ...
      return renderPreviewSvgScene({ content });
    }
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function createCheckoutCompositePreviewUiSourceUrl() {
      ...
      try {
        await ensureSelectedPreviewFontReady(selectedTemplate);
      } catch (_) {}
    ...
    async function createCheckoutCompositePreviewFile(type = 'image/jpeg', quality = 0.92) {
      return withLiveCheckoutPreviewScene(async () => {
        ...
        try {
          ...
          // Explicit selected font loading is best-effort for export fidelity.
        }
        await waitForCurrentBaseImageReady();
        const isIosSafari = isLikelyIosWebkit();
        const preferCanvasPreview = templateSupportsText(selectedTemplate) || isPolaroidPhotoTemplate(selectedTemplate);
        let canvasPreview = null;
        if (preferCanvasPreview || isIosSafari) {
          ...
          if (canvasPreview.file) {
            return {
              ...canvasPreview,
              kind: preferCanvasPreview ? 'raster-canvas-text' : 'raster-canvas-ios',
            };
          }
        }
        if (!isIosSafari) {
