...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    async function ensureSelectedPreviewFontReady(template = getSelectedTemplate()) {
      ...
      const previewLines = getPreviewTextLines(state.textValue || previewTextPlaceholder, template, selectedPrintSize);
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function getCheckoutCompositePreviewUrl(options = {}) {
      ...
      let previewUrl = '';
      if (isPhotoTextTemplate(selectedTemplate) || isPolaroidPhotoTemplate(selectedTemplate) || isSimplePhotoTemplate(selectedTemplate)) {
        const canvasPreview = await withLiveCheckoutPreviewScene(() => createCheckoutCompositePreviewFileFromCanvas('image/png'));
        previewUrl = canvasPreview.dataUrl || '';
      }
      if (!previewUrl) {
    ...
    function shouldUseCheckoutCompositePreviewInUi(template = getSelectedTemplate()) {
      ...
      return isPolaroidInsideTextTemplate(template);
    }
...
function renderLeftSummary() {
    ...
    function getDesignLayerRenderState() {
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        `;
        const photoBlockHtml = activeUpload ? `
          <div
            class="image-box"
