...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const KONVA_TEMPLATE_CONFIGS = {
      ...
      'polaroid-top-text': {
        ...
        textDefaults: { exportX: 500, exportCenterY: 500, anchor: 'middle' },
        textGap: { preview: 16, export: 20 }, textDescentRatio: 0.32,
        layerOrder: ['text', 'photo', 'polaroidFrame'],
      },
      'polaroid-bottom-text': {
        hasText: true, hasPhoto: true, hasFrame: false, isPolaroid: true,
        textPosition: 'below-photo', photoPosition: 'center',
    ...
    function konvaResolveScene(params) {
      ...
      for (const layerName of (config.layerOrder || [])) {
          ...
          if (layerName === 'standardUpload' && config.hasStandardUpload && hasUpload && photoSrc) {
            ...
            if (lay) {
              if (hasUpload && photoSrc) {
                ...
                scene.elements.push({ type: 'photo-placeholder', name: 'photoPlaceholder', x: lay.x, y: lay.y, width: lay.width, height: lay.height, polaroid: Boolean(config.isPolaroid) });
              }
            }
          }
        ...
        } else if (layerName === 'polaroidFrame' && config.isPolaroid && frameSrc && photoLayout) {
          scene.elements.push({ type: 'frame', name: 'polaroidFrame', x: photoLayout.x, y: photoLayout.y, width: photoLayout.width, height: photoLayout.height, src: frameSrc });
...
function renderLeftSummary() {
    ...
    function getDesignLayerRenderState() {
      ...
      if (!hideDesign && state.selectedCustom) {
        html = '';
      } else if (!hideDesign && (isPhotoTextTemplate(selectedTemplate) || isPolaroidTextTemplate(selectedTemplate)) && templatePhotoLayout) {
        const textBlockHtml = `
