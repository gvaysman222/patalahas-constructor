    ...
    function getPreviewTextBlockStyleConfig(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1) {
          ...
          if (template?.id === 'emb-center') {
            ...
            if (Number.isFinite(baseTop)) {
              previewTop = `${Math.max(30, baseTop - (Math.max(0, lineCount - 1) * 1.1))}%`;
            }
          }
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        html = `
          <div class="polaroid-wrap" style="--print-scale:${currentScale}; --text-color:${state.textColor}; --tilt:${textTilt ? '-8deg' : '0deg'}; font-family:${previewFontFamily};">
            ${textTop ? `<div class="polaroid-text" style="-webkit-line-clamp:${previewTextRules.maxLines};">${previewTextHtml}</div>` : ''}
            <div class="polaroid-frame">
              <div class="polaroid-photo">${uploadsHtml}</div>
              <div class="polaroid-gap"></div>
            </div>
            ${textBottom || textTilt ? `<div class="polaroid-text" style="margin-top:8px; -webkit-line-clamp:${previewTextRules.maxLines};">${previewTextHtml}</div>` : ''}
          </div>
        `;
      } else if (!hideDesign) {
        ...
        html = `
          ...
          <div class="emb-block ${isEmbLeft ? 'left' : ''} ${hasUpload ? 'with-upload' : ''}" style="--print-scale:${currentScale}; --text-color:${state.textColor}; --preview-text-max-width:${previewTextStyle.maxWidth}; --preview-text-line-height:${previewTextStyle.lineHeight}; --preview-text-font-weight:${previewTextStyle.fontWeight}; --preview-text-letter-spacing:${previewTextStyle.letterSpacing}; --preview-text-top:${previewTextStyle.previewTop}; --preview-text-transform:${previewTextStyle.textTransform}; font-size:${previewTextStyle.fontSize}; font-family:${previewFontFamily}; -webkit-line-clamp:${previewTextRules.maxLines};">
            ${previewTextHtml}
          </div>
        `;
      }
