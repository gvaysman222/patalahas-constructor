...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const POLAROID_FRAME_IMAGE_URL = 'https://www.dropbox.com/scl/fo/04y8ovljbyi645ppvs0xe/AAZUhi-Z6nvGh707riBYscA/rama.png?rlkey=pnn5zy5e82rhap6fbta5ks53v&st=nyafkglu&dl=0';
    ...
    const KONVA_TEMPLATE_CONFIGS = {
      ...
      'simple-photo': {
        ...
        photoDefaults: { exportX: 455, exportY: 485, scale: 0.8 },
        printSizeOverrides: {
          ...
          if (layerName === 'standardUpload' && config.hasStandardUpload && hasUpload && photoSrc) {
            ...
            if (photoLayout) {
              if (hasUpload && photoSrc) {
                ...
                scene.elements.push({ type: 'photo-placeholder', name: 'photoPlaceholder', x: photoLayout.x, y: photoLayout.y, width: photoLayout.width, height: photoLayout.height, polaroid: Boolean(config.isPolaroid) });
              }
            }
          }
    ...
    async function preloadInitialRenderImages() {
      ...
      // Preload polaroid frame into Konva image cache
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function createCheckoutCompositePreviewUrl(options = {}) {
      ...
      if (state.selectedCustom) {
        ...
        if ((isPhotoTextTemplate(selectedTemplate) || isPolaroidTextTemplate(selectedTemplate)) && templatePhotoLayout) {
          ...
          const photoMarkup = standardUploadPreviewUrl ? `
            ...
            />
          ` : '';
          const frameMarkup = polaroidFrameHref ? `
            <image
          ...
          designMarkup = isPhotoFirst
            ...
            : `${textMarkup}${photoMarkup}${frameMarkup}`;
        } else if (templatePhotoLayout) {
          const photoOnly = standardUploadPreviewUrl ? `
            <image
              href="${escapeSvg(standardUploadPreviewUrl)}"
              x="${templatePhotoLayout.exportX}"
              y="${templatePhotoLayout.exportY}"
            ...
            />
          ` : '';
          const frameOnly = polaroidFrameHref ? `
            <image
              href="${escapeSvg(polaroidFrameHref)}"
              x="${templatePhotoLayout.exportX}"
              y="${templatePhotoLayout.exportY}"
            ...
            />
          ` : '';
          designMarkup = `${photoOnly}${frameOnly}`;
        } else {
          const textMetrics = getPreviewTextRenderMetrics(selectedTemplate, selectedPrintSize, textLines.length, {
            hasStandardUpload,
          });
      ...
      if ((isPhotoTextTemplate(selectedTemplate) || isPolaroidTextTemplate(selectedTemplate)) && templatePhotoLayout) {
        ...
        if (hasStandardUpload) {
          ...
          drawCanvasPhotoPlaceholder(
            ...
            templatePhotoLayout.exportWidth,
            templatePhotoLayout.exportHeight,
            { radius: isPolaroidTemplate ? 0 : 14 }
          );
        }
        if (isPolaroidTemplate) {
          const polaroidFrameSource = await fetchImageAsDataUrl(POLAROID_FRAME_IMAGE_URL);
          await drawCanvasContainImage(ctx, polaroidFrameSource, templatePhotoLayout.exportX, templatePhotoLayout.exportY, templatePhotoLayout.exportWidth, templatePhotoLayout.exportHeight);
        }
...
function renderLeftSummary() {
    ...
    function renderVisualizer() {
      ...
      if (isKonvaAvailable()) {
        ...
        const sceneParams = buildKonvaSceneParams();
        const overlayParams = { ...sceneParams, baseImageSrc: null, frameSrc: state.step <= 2 ? null : sceneParams.frameSrc };
