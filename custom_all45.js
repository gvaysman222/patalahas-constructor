...
(function initConstructorCheckoutTemplateConfig(global) {
  ...
  global.CONSTRUCTOR_CHECKOUT_TEMPLATE_CONFIG = {
    templates: [
      ...
      { id: 'polaroid', name: 'Просто фото + полароид', uploads: 1, kind: 'polaroid', price: 300 },
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function getPolaroidFrameLayout() {
      const exportX = (CHECKOUT_EXPORT_ARTBOARD_WIDTH - POLAROID_FRAME_EXPORT_WIDTH) / 2;
    ...
    function getGarmentPreviewFocusConfig(template = getSelectedTemplate()) {
      ...
      if (isPhotoTextTemplate(template)) {
        return {
          ...
          focusX: 0.5,
          focusY: (isPhotoBottomTextTemplate(template) || isPhotoDiagonalTextTemplate(template)) ? 0.46 : 0.45,
        };
      }
    ...
    function getPreviewTextBlockStyleConfig(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1) {
      ...
      if (isPhotoDiagonalTextTemplate(template)) {
        ...
        const photoTopPercent = Number.parseFloat(String(photoLayout?.previewTop || '').replace('%', ''));
        const photoHeightPercent = Number.parseFloat(String(photoLayout?.previewHeight || '').replace('%', ''));
        if (Number.isFinite(photoTopPercent) && Number.isFinite(photoHeightPercent)) {
          previewTop = `${photoTopPercent + photoHeightPercent + ((PHOTO_BOTTOM_TEXT_PREVIEW_GAP / GARMENT_ARTBOARD_HEIGHT) * 100)}%`;
        }
      }
    ...
    function getPreviewTextRenderMetrics(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1, options = {}) {
      ...
      if (config) {
        ...
        if (isPhotoBottomTextTemplate(template)) {
          ...
          const textTop = Number(photoLayout?.exportY || 0)
            + Number(photoLayout?.exportHeight || 0)
    ...
    function templateSupportsText(template = getSelectedTemplate()) {
      if (!template || isCustomSelected()) return false;
      return ['emb-center', 'emb-left', 'photo-top-text', 'photo-bottom-text', 'photo-diagonal-text'].includes(template.kind);
    }
    ...
    function isStepComplete(stepId) {
      ...
      if (stepId === 4) {
        ...
        if (isPolaroidPhotoTemplate(template)) return true;
...
// ===== constructor-checkout-mvp-tilda-js-2-steps.html =====
    ...
    function renderStepContent() {
      ...
      if (state.step === 4) {
        ...
        if (isPolaroidPhotoTemplate(selectedTemplate)) {
          container.innerHTML = `
            <div class="section">
              <div class="muted-box">
                Для этого шаблона дополнительная настройка не нужна. Загрузите фото на 3 шаге, и мы поместим его внутрь рамки полароида.
              </div>
            </div>
          `;
          return;
