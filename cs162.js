...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    // =====================================================================
    const KONVA_DEV_MODE = false; // <<< set true for development
    ...
    // =====================================================================
    // PREVIEW ZOOM (step 4 — desktop hover zoom + mobile pinch zoom)
    // =====================================================================
    ...
    function setupPreviewZoom() {
      ...
      // --- Desktop: hover = zoom, mousemove = pan via translate (loupe effect) ---
      const isDesktop = !window.matchMedia('(max-width: 768px)').matches;
      if (isDesktop) {
        // Origin always center; we move the zoomed content via translate
        // so the area under cursor stays visible
        garmentWrap.style.transformOrigin = '50% 50%';

        const applyDesktopZoom = (e, animate) => {
          const parent = garmentWrap.parentElement;
          const rect = parent.getBoundingClientRect();
          // How far the cursor is from center, as fraction (-0.5 to 0.5)
          const fx = (e.clientX - rect.left) / rect.width - 0.5;
          const fy = (e.clientY - rect.top) / rect.height - 0.5;
          // Translate to keep cursor area in view
          // At zoom N, the overflow is (N-1)*size. Move proportionally to cursor offset.
      ...
      const applyTransform = (ox, oy, zoom, tx, ty, animate) => {
        garmentWrap.style.transition = animate ? 'transform .25s ease' : 'none';
        garmentWrap.style.transformOrigin = `${ox}% ${oy}%`;
        garmentWrap.style.transform = `translate(${tx}px, ${ty}px) scale(${zoom})`;
        currentZoom = zoom;
        panOffset = { x: tx, y: ty };
      };
      ...
      if (!isDesktop) {
        ...
        const getTouchDist = (a, b) => Math.hypot(b.clientX - a.clientX, b.clientY - a.clientY);
        ...
        garmentWrap.addEventListener('touchmove', (e) => {
          if (e.touches.length === 2) {
            ...
            const newZoom = Math.max(1, Math.min(6, pinchStartZoom * (dist / pinchStartDist)));
            const dx = lastTouchCenter ? center.x - lastTouchCenter.x : 0;
            const dy = lastTouchCenter ? center.y - lastTouchCenter.y : 0;
            lastTouchCenter = center;
            applyTransform(50, 50, newZoom, panOffset.x + dx, panOffset.y + dy, false);
          } else if (e.touches.length === 1 && panStart && currentZoom > 1) {
            e.preventDefault();
            const px = e.touches[0].clientX - panStart.x;
            const py = e.touches[0].clientY - panStart.y;
            applyTransform(50, 50, currentZoom, px, py, false);
          }
        }, { signal, passive: false });
        ...
        garmentWrap.addEventListener('touchend', (e) => {
          ...
          panStart = null;
          if (e.touches.length === 0 && currentZoom <= 1.05) {
            resetTransform(true);
          }
        }, { signal });
    ...
    function getGarmentPreviewFocusConfig(template = getSelectedTemplate()) {
      // Preview zoom is now handled by setupPreviewZoom (hover/pinch)
      return null;
          ...
          <div class="checkout-review-grid">
            ...
            <div class="info-card production-card">
              <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
                ...
                <div class="status-pill ${state.fastProduction ? 'ok' : ''}" style="margin:0;">${state.fastProduction ? '✓ Ускорение включено' : 'Стандартное производство'}</div>
              </div>
