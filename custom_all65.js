...
(function initConstructorCheckoutTemplateConfig(global) {
  ...
  const polaroidInsideTextPrintSizes = centerTextPrintSizes.map((size) => ({
    ...size,
    maxLines: 3,
    previewMaxWidth: size.id === 's' ? 92 : 104,
    previewFontSize: size.id === 's' ? 4.6 : 5.4,
    exportFontSize: size.id === 's' ? 5.2 : 6.2,
    exportLineHeight: size.id === 's' ? 6.2 : 7.4,
    lineHeight: 1.1,
    fontWeight: 700,
    letterSpacing: '0',
    previewTop: '50%',
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function getPreviewTextBlockStyleConfig(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1) {
      ...
      if (isPolaroidInsideTextTemplate(template)) {
        ...
        const previewTextHeight = basePreviewFontSize * previewFontScale * previewFocusScale
      ...
      return {
        ...
        maxWidth: `${Math.round(rules.previewMaxWidth * previewWidthScale * previewFocusScale)}px`,
        ...
        fontSize: `${(Math.round(basePreviewFontSize * previewFontScale * previewFocusScale * 100) / 100)}px`,
      };
    }
...
function renderLeftSummary() {
    ...
    async function renderPolaroidPreviewCanvasFromDom(ctx, domMetrics) {
      ...
      const textBlocks = Array.from(designLayer.querySelectorAll('.polaroid-text')).filter((node) => node instanceof HTMLElement);
      ...
      for (const element of textBlocks) {
        await withCanvasElementTransform(ctx, element, domMetrics, async ({ width, height }) => {
          if (element.classList.contains('polaroid-text-inside')) {
            ctx.save();
            buildRoundedRectPath(ctx, 0, 0, width, height, 0);
            ctx.clip();
          }
    ...
    function getDesignLayerRenderState() {
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        const insideTextBlockHtml = polaroidInsideTextLayout ? `
          ...
          >
            <div class="polaroid-text polaroid-text-inside" style="width:100%; height:100%; max-width:none; box-sizing:border-box; padding:${polaroidInsideTextLayout.localPaddingY} ${polaroidInsideTextLayout.localPaddingX}; color:${state.textColor}; text-align:right; line-height:${Math.round(((Number(polaroidInsideTextMetrics?.lineHeight || 0) / Math.max(1, Number(polaroidInsideTextMetrics?.fontSize || 1))) * 1000)) / 1000}; font-weight:${previewTextStyle.fontWeight}; letter-spacing:${previewTextStyle.letterSpacing}; text-transform:${previewTextStyle.textTransform}; font-size:${Math.round(((Number(polaroidInsideTextMetrics?.fontSize || 0) / Math.max(1, Number(polaroidInsideTextLayout.exportHeight || 1))) * 10000)) / 100}%; font-family:${previewFontFamily}; white-space:normal; overflow:visible; overflow-wrap:anywhere; word-break:break-word;">
              ${previewTextHtml}
            </div>
          </div>
        ` : '';
